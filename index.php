<?php
include 'includes/header.php';
?>

    <!-- home1 Banner Section Start-->
    <div class="home1-banner-section">
        <div class="banner-video-area">
            <video autoplay loop muted playsinline src="assets/video/home1-banner-video.mp4"></video>
        </div>
        <div class="banner-content-wrap">
            <div class="container">
                <div class="banner-content">
                    <h1>EXPERIENCE YOU CAN TRUST</h1>
                    <p>No one knows West Africa better than our team of experienced Africa travel experts strategically located throughout the region</p>
                </div>
            </div>
        </div>
    </div>
    <div class="filter-wrapper mb-100">
        <div class="container">
            <ul class="filter-item-list">
                <li class="single-item active">
                    <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <g>
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M15.0698 22.0465V23.1628C15.0698 23.3848 15.158 23.5978 15.315 23.7548C15.472 23.9118 15.6849 24 15.907 24C16.129 24 16.342 23.9118 16.499 23.7548C16.656 23.5978 16.7442 23.3848 16.7442 23.1628V22.0465C16.7442 21.8245 16.656 21.6115 16.499 21.4545C16.342 21.2975 16.129 21.2093 15.907 21.2093C15.6849 21.2093 15.472 21.2975 15.315 21.4545C15.158 21.6115 15.0698 21.8245 15.0698 22.0465ZM13.3953 0.837209V6.4186C13.3953 6.64065 13.4835 6.85359 13.6405 7.0106C13.7976 7.16761 14.0105 7.25581 14.2325 7.25581C14.4546 7.25581 14.6675 7.16761 14.8245 7.0106C14.9815 6.85359 15.0698 6.64065 15.0698 6.4186V0.837209C15.0698 0.615168 14.9815 0.40222 14.8245 0.245213C14.6675 0.0882058 14.4546 0 14.2325 0C14.0105 0 13.7976 0.0882058 13.6405 0.245213C13.4835 0.40222 13.3953 0.615168 13.3953 0.837209ZM7.2558 22.0465V23.1628C7.2558 23.3848 7.344 23.5978 7.50101 23.7548C7.65802 23.9118 7.87097 24 8.09301 24C8.31505 24 8.528 23.9118 8.685 23.7548C8.84201 23.5978 8.93022 23.3848 8.93022 23.1628V22.0465C8.93022 21.8245 8.84201 21.6115 8.685 21.4545C8.528 21.2975 8.31505 21.2093 8.09301 21.2093C7.87097 21.2093 7.65802 21.2975 7.50101 21.4545C7.344 21.6115 7.2558 21.8245 7.2558 22.0465ZM8.93022 0.837209V6.4186C8.93022 6.64065 9.01842 6.85359 9.17543 7.0106C9.33244 7.16761 9.54538 7.25581 9.76743 7.25581C9.98947 7.25581 10.2024 7.16761 10.3594 7.0106C10.5164 6.85359 10.6046 6.64065 10.6046 6.4186V0.837209C10.6046 0.615168 10.5164 0.40222 10.3594 0.245213C10.2024 0.0882058 9.98947 0 9.76743 0C9.54538 0 9.33244 0.0882058 9.17543 0.245213C9.01842 0.40222 8.93022 0.615168 8.93022 0.837209Z"/>
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M8.65113 1.67442H15.3488C15.5709 1.67442 15.7838 1.58621 15.9408 1.42921C16.0978 1.2722 16.186 1.05925 16.186 0.837209C16.186 0.615168 16.0978 0.40222 15.9408 0.245213C15.7838 0.0882057 15.5709 0 15.3488 0H8.65113C8.42909 0 8.21615 0.0882057 8.05914 0.245213C7.90213 0.40222 7.81392 0.615168 7.81392 0.837209C7.81392 1.05925 7.90213 1.2722 8.05914 1.42921C8.21615 1.58621 8.42909 1.67442 8.65113 1.67442ZM19.5349 8.65116C19.5349 7.83701 19.2114 7.0562 18.6357 6.48051C18.06 5.90482 17.2792 5.5814 16.4651 5.5814H7.53486C6.7207 5.5814 5.93989 5.90482 5.3642 6.48051C4.78851 7.0562 4.46509 7.83701 4.46509 8.65116V19.814C4.46509 20.6281 4.78851 21.4089 5.3642 21.9846C5.93989 22.5603 6.7207 22.8837 7.53486 22.8837H16.4651C17.2792 22.8837 18.06 22.5603 18.6357 21.9846C19.2114 21.4089 19.5349 20.6281 19.5349 19.814V8.65116ZM11.1628 9.76744V18.6977C11.1628 18.9197 11.251 19.1327 11.408 19.2897C11.565 19.4467 11.7779 19.5349 12 19.5349C12.222 19.5349 12.435 19.4467 12.592 19.2897C12.749 19.1327 12.8372 18.9197 12.8372 18.6977V9.76744C12.8372 9.5454 12.749 9.33245 12.592 9.17545C12.435 9.01844 12.222 8.93023 12 8.93023C11.7779 8.93023 11.565 9.01844 11.408 9.17545C11.251 9.33245 11.1628 9.5454 11.1628 9.76744ZM7.81392 9.76744V18.6977C7.81392 18.9197 7.90213 19.1327 8.05914 19.2897C8.21615 19.4467 8.42909 19.5349 8.65113 19.5349C8.87318 19.5349 9.08612 19.4467 9.24313 19.2897C9.40014 19.1327 9.48834 18.9197 9.48834 18.6977V9.76744C9.48834 9.5454 9.40014 9.33245 9.24313 9.17545C9.08612 9.01844 8.87318 8.93023 8.65113 8.93023C8.42909 8.93023 8.21615 9.01844 8.05914 9.17545C7.90213 9.33245 7.81392 9.5454 7.81392 9.76744ZM14.5116 9.76744V18.6977C14.5116 18.9197 14.5998 19.1327 14.7568 19.2897C14.9138 19.4467 15.1268 19.5349 15.3488 19.5349C15.5709 19.5349 15.7838 19.4467 15.9408 19.2897C16.0978 19.1327 16.186 18.9197 16.186 18.6977V9.76744C16.186 9.5454 16.0978 9.33245 15.9408 9.17545C15.7838 9.01844 15.5709 8.93023 15.3488 8.93023C15.1268 8.93023 14.9138 9.01844 14.7568 9.17545C14.5998 9.33245 14.5116 9.5454 14.5116 9.76744Z"/>
                        </g>
                    </svg>
                    <span>Tours</span>
                </li>
                <li class="single-item hotel">
                    <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M21.5381 12.8663L20.7881 12.2344V7.91437H21.5381V12.8663ZM23.595 6.64125L22.3744 3.78937C22.3451 3.72215 22.2969 3.66489 22.2356 3.62461C22.1744 3.58432 22.1027 3.56274 22.0294 3.5625H20.2969C20.2237 3.56253 20.1522 3.58405 20.0912 3.62438C20.0302 3.66472 19.9824 3.72209 19.9538 3.78937L18.7331 6.64125C18.7087 6.69834 18.6989 6.76059 18.7044 6.82242C18.71 6.88426 18.7308 6.94375 18.765 6.99558C18.7991 7.04741 18.8456 7.08995 18.9003 7.11941C18.9549 7.14887 19.016 7.16432 19.0781 7.16437H23.25C23.3121 7.16432 23.3732 7.14887 23.4278 7.11941C23.4825 7.08995 23.529 7.04741 23.5632 6.99558C23.5973 6.94375 23.6181 6.88426 23.6237 6.82242C23.6293 6.76059 23.6194 6.69834 23.595 6.64125ZM6.05625 10.9875V10.5862C6.05689 10.3199 6.16298 10.0647 6.35132 9.87632C6.53965 9.68798 6.79491 9.58189 7.06125 9.58125H9.44062C9.70706 9.5816 9.96249 9.68759 10.1509 9.87599C10.3393 10.0644 10.4453 10.3198 10.4456 10.5862V10.9875H12.0544V10.5862C12.0547 10.3198 12.1607 10.0644 12.3491 9.87599C12.5375 9.68759 12.7929 9.5816 13.0594 9.58125H15.4387C15.7051 9.58189 15.9603 9.68798 16.1487 9.87632C16.337 10.0647 16.4431 10.3199 16.4437 10.5862V10.9875H18.375V9.02625C18.3743 8.47798 18.1561 7.95237 17.7684 7.56469C17.3808 7.177 16.8551 6.95887 16.3069 6.95813H6.19312C5.64485 6.95887 5.11925 7.177 4.73156 7.56469C4.34387 7.95237 4.12574 8.47798 4.125 9.02625V10.9875H6.05625ZM20.7206 14.1375H21.8869L21.5381 13.845L20.7881 13.215L19.0294 11.7375H3.47062L0.613125 14.1375H20.7206ZM2.0625 19.1794H1.3125V20.0625C1.3128 20.1619 1.3524 20.2571 1.42266 20.3273C1.49292 20.3976 1.58814 20.4372 1.6875 20.4375H3.51375C3.61311 20.4372 3.70833 20.3976 3.77859 20.3273C3.84885 20.2571 3.88845 20.1619 3.88875 20.0625V19.1794H2.0625ZM19.3612 19.1794H18.6112V20.0625C18.6115 20.1619 18.6512 20.2571 18.7214 20.3273C18.7917 20.3976 18.8869 20.4372 18.9862 20.4375H20.8125C20.9119 20.4372 21.0071 20.3976 21.0773 20.3273C21.1476 20.2571 21.1872 20.1619 21.1875 20.0625V19.1794H19.3612ZM1.125 14.8875H0.375V18.0544C0.375297 18.1537 0.414901 18.249 0.485163 18.3192C0.555425 18.3895 0.650635 18.4291 0.75 18.4294H21.75C21.8494 18.4291 21.9446 18.3895 22.0148 18.3192C22.0851 18.249 22.1247 18.1537 22.125 18.0544V14.8875H1.125Z"/>
                    </svg>
                    <span>Hotels</span>
                </li>
                <li class="single-item visa">
                    <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <g>
                            <path
                                d="M16.9221 1.23284C16.9197 1.03856 16.8717 0.847553 16.782 0.675228C16.6922 0.502902 16.5632 0.354077 16.4054 0.240751C16.244 0.122177 16.0563 0.0444497 15.8583 0.0142152C15.6604 -0.0160194 15.458 0.0021383 15.2686 0.0671357L7.1955 2.88632H16.9221V1.23284ZM12.375 15.0808C12.5734 15.0808 13.1232 14.0887 13.2018 12.1872H11.5483C11.6268 14.0887 12.1766 15.0808 12.375 15.0808ZM12.375 8.46682C12.1766 8.46682 11.6268 9.45891 11.5483 11.3604H13.2018C13.1232 9.45891 12.5734 8.46682 12.375 8.46682ZM11.2341 14.8741C10.909 14.0149 10.7355 13.1057 10.7215 12.1872H9.097C9.17316 12.7877 9.41272 13.3559 9.78957 13.8297C10.1664 14.3035 10.6661 14.6647 11.2341 14.8741ZM11.2341 8.67351C10.6661 8.88285 10.1664 9.24411 9.78957 9.7179C9.41272 10.1917 9.17316 10.7598 9.097 11.3604H10.7215C10.7355 10.4419 10.909 9.53271 11.2341 8.67351Z"/>
                            <path
                                d="M4.52094 4.95319V23.1415C4.52094 23.4704 4.65159 23.7858 4.88416 24.0184C5.11672 24.2509 5.43215 24.3816 5.76105 24.3816H18.9889C19.3178 24.3816 19.6332 24.2509 19.8658 24.0184C20.0984 23.7858 20.229 23.4704 20.229 23.1415V4.95319C20.229 4.62429 20.0984 4.30886 19.8658 4.07629C19.6332 3.84373 19.3178 3.71307 18.9889 3.71307H5.76105C5.43215 3.71307 5.11672 3.84373 4.88416 4.07629C4.65159 4.30886 4.52094 4.62429 4.52094 4.95319ZM15.6819 20.4546H9.06801C8.95838 20.4546 8.85324 20.411 8.77571 20.3335C8.69819 20.256 8.65464 20.1508 8.65464 20.0412C8.65464 19.9316 8.69819 19.8264 8.77571 19.7489C8.85324 19.6714 8.95838 19.6278 9.06801 19.6278H15.6819C15.7916 19.6278 15.8967 19.6714 15.9742 19.7489C16.0518 19.8264 16.0953 19.9316 16.0953 20.0412C16.0953 20.1508 16.0518 20.256 15.9742 20.3335C15.8967 20.411 15.7916 20.4546 15.6819 20.4546ZM10.3081 18.8011C10.3081 18.6915 10.3517 18.5863 10.4292 18.5088C10.5067 18.4313 10.6119 18.3877 10.7215 18.3877H14.0285C14.1381 18.3877 14.2432 18.4313 14.3208 18.5088C14.3983 18.5863 14.4418 18.6915 14.4418 18.8011C14.4418 18.9107 14.3983 19.0159 14.3208 19.0934C14.2432 19.1709 14.1381 19.2145 14.0285 19.2145H10.7215C10.6119 19.2145 10.5067 19.1709 10.4292 19.0934C10.3517 19.0159 10.3081 18.9107 10.3081 18.8011ZM12.375 7.64009C13.1925 7.64009 13.9918 7.88253 14.6715 8.33675C15.3513 8.79097 15.8811 9.43656 16.194 10.1919C16.5069 10.9472 16.5888 11.7784 16.4293 12.5802C16.2698 13.3821 15.8761 14.1187 15.2979 14.6968C14.7198 15.2749 13.9833 15.6686 13.1814 15.8281C12.3796 15.9876 11.5484 15.9057 10.7931 15.5928C10.0377 15.28 9.39214 14.7501 8.93792 14.0704C8.48371 13.3906 8.24127 12.5914 8.24127 11.7738C8.24127 10.6775 8.67678 9.62605 9.452 8.85083C10.2272 8.07561 11.2786 7.64009 12.375 7.64009Z"/>
                            <path
                                d="M15.653 11.3604C15.5768 10.7599 15.3373 10.1917 14.9604 9.71792C14.5836 9.24412 14.0839 8.88287 13.5159 8.67352C13.841 9.53272 14.0145 10.4419 14.0284 11.3604H15.653ZM15.653 12.1872H14.0284C14.0145 13.1057 13.841 14.0149 13.5159 14.8741C14.0839 14.6647 14.5836 14.3035 14.9604 13.8297C15.3373 13.3559 15.5768 12.7877 15.653 12.1872Z"/>
                        </g>
                    </svg>
                    <span>Visa</span>
                </li>
               
            </ul>
            <div class="filter-input-wrap">
                <form class="filter-input show">
                    <div class="single-search-box">
                        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M12.5944 8.99987C12.5944 10.988 10.9826 12.5998 8.99443 12.5998C7.00627 12.5998 5.39465 10.988 5.39465 8.99987C5.39465 7.0117 7.00627 5.40009 8.99443 5.40009C10.9826 5.40009 12.5944 7.0117 12.5944 8.99987Z"/>
                                <path
                                    d="M17.4601 8.4599H16.2564C15.9858 4.86535 13.1291 2.00812 9.53458 1.7372V0.539976C9.53458 0.241723 9.29268 0 8.9946 0C8.69635 0 8.45462 0.241723 8.45462 0.539976V1.7372C4.85986 2.00812 2.00297 4.86535 1.73235 8.4599H0.540018C0.241723 8.4599 0 8.7017 0 8.99987C0 9.29813 0.241723 9.53985 0.539976 9.53985H1.73239C2.00297 13.1344 4.85991 15.9916 8.45441 16.2625V17.4601C8.45441 17.7583 8.69614 18 8.99439 18C9.29251 18 9.53428 17.7583 9.53428 17.4601V16.2625C13.1289 15.9918 15.9858 13.1346 16.2564 9.53985H17.4601C17.7583 9.53985 18 9.29813 18 8.99987C18 8.70175 17.7583 8.4599 17.4601 8.4599ZM8.99443 15.2096C5.56504 15.2094 2.78509 12.4291 2.78509 8.9997C2.78522 5.57014 5.56554 2.7902 8.99494 2.7902C12.4245 2.7902 15.2046 5.57048 15.2046 8.99987C15.2005 12.428 12.4225 15.2058 8.99443 15.2096Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown destination-dropdown">
                            <input type="text" readonly value="Where are you going?">
                            <div class="input-field-value">
                                <div class="destination">
                                    <h6>Ghana</h6>
                                    <span>West Africa</span>
                                </div> 
                            </div>
                        </div>
                        <div class="custom-select-wrap">
                            <div class="custom-select-search-area">
                                <i class='bx bx-search'></i>
                                <input type="text" placeholder="Type Your Destination">
                            </div>
                            <ul class="option-list-destination">
                                <li>
                                    <div class="destination">
                                        <h6>Egypt</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Morocco</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Algeria</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Tunisia</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Libya</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Sudan</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Nigeria</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ghana</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Senegal</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ivory Coast</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mali</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Niger</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Cameroon</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Democratic Republic of the Congo</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Gabon</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Central African Republic</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Chad</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Equatorial Guinea</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Kenya</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Tanzania</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Uganda</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ethiopia</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Rwanda</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Somalia</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>South Africa</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Zimbabwe</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Botswana</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Namibia</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Zambia</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mozambique</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mauritius</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Seychelles</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Madagascar</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Comoros</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Réunion</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Maldives</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="single-search-box date-field">
                        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M16.125 1.28394H14.8913V2.43609C14.9509 2.57307 14.9755 2.72275 14.9629 2.87163C14.9502 3.0205 14.9007 3.16388 14.8188 3.28883C14.7368 3.41379 14.6251 3.51638 14.4936 3.58736C14.3622 3.65834 14.2151 3.69547 14.0657 3.6954C13.9163 3.69533 13.7692 3.65807 13.6378 3.58697C13.5064 3.51587 13.3948 3.41318 13.313 3.28815C13.2312 3.16312 13.1818 3.0197 13.1693 2.87081C13.1567 2.72193 13.1815 2.57227 13.2413 2.43534V1.28409H11.5136V2.43609C11.5733 2.57304 11.598 2.72272 11.5854 2.87159C11.5728 3.02047 11.5234 3.16388 11.4415 3.28887C11.3597 3.41386 11.248 3.5165 11.1165 3.58754C10.9851 3.65858 10.838 3.69577 10.6886 3.69577C10.5392 3.69577 10.3922 3.65858 10.2607 3.58754C10.1293 3.5165 10.0176 3.41386 9.93572 3.28887C9.85387 3.16388 9.80441 3.02047 9.79183 2.87159C9.77924 2.72272 9.80391 2.57304 9.86363 2.43609V1.28394H8.13638V2.43609C8.19609 2.57304 8.22076 2.72272 8.20818 2.87159C8.19559 3.02047 8.14613 3.16388 8.06428 3.28887C7.98242 3.41386 7.87073 3.5165 7.73929 3.58754C7.60784 3.65858 7.46079 3.69577 7.31138 3.69577C7.16197 3.69577 7.01491 3.65858 6.88346 3.58754C6.75202 3.5165 6.64033 3.41386 6.55848 3.28887C6.47662 3.16388 6.42716 3.02047 6.41457 2.87159C6.40199 2.72272 6.42666 2.57304 6.48638 2.43609V1.28394H4.75875V2.43519C4.81852 2.57212 4.84327 2.72178 4.83075 2.87066C4.81823 3.01955 4.76884 3.16297 4.68704 3.288C4.60524 3.41303 4.49359 3.51572 4.36219 3.58682C4.23078 3.65792 4.08373 3.69518 3.93432 3.69525C3.78491 3.69532 3.63784 3.65819 3.50636 3.58721C3.37489 3.51623 3.26315 3.41364 3.18124 3.28868C3.09932 3.16373 3.0498 3.02035 3.03715 2.87148C3.02449 2.7226 3.0491 2.57292 3.10875 2.43594V1.28394H1.875C1.37772 1.28394 0.900806 1.48148 0.549175 1.83311C0.197544 2.18474 0 2.66165 0 3.15894L0 16.0964C4.97191e-05 16.5937 0.19761 17.0706 0.54923 17.4222C0.90085 17.7738 1.37773 17.9714 1.875 17.9714H16.125C16.6223 17.9714 17.0992 17.7738 17.4508 17.4222C17.8024 17.0706 18 16.5937 18 16.0964V3.15894C18 2.66165 17.8025 2.18474 17.4508 1.83311C17.0992 1.48148 16.6223 1.28394 16.125 1.28394ZM17.25 15.9089C17.25 16.257 17.1117 16.5909 16.8656 16.837C16.6194 17.0832 16.2856 17.2214 15.9375 17.2214H2.0625C1.7144 17.2214 1.38056 17.0832 1.13442 16.837C0.888281 16.5909 0.75 16.257 0.75 15.9089V6.34644C0.75 5.99834 0.888281 5.6645 1.13442 5.41836C1.38056 5.17222 1.7144 5.03394 2.0625 5.03394H15.9375C16.2856 5.03394 16.6194 5.17222 16.8656 5.41836C17.1117 5.6645 17.25 5.99834 17.25 6.34644V15.9089Z"/>
                                <path
                                    d="M14.6287 0.591064C14.6287 0.280404 14.3769 0.0285645 14.0662 0.0285645C13.7556 0.0285645 13.5037 0.280404 13.5037 0.591064V2.84106C13.5037 3.15172 13.7556 3.40356 14.0662 3.40356C14.3769 3.40356 14.6287 3.15172 14.6287 2.84106V0.591064Z"/>
                                <path
                                    d="M11.2512 0.591064C11.2512 0.280404 10.9994 0.0285645 10.6887 0.0285645C10.3781 0.0285645 10.1262 0.280404 10.1262 0.591064V2.84106C10.1262 3.15172 10.3781 3.40356 10.6887 3.40356C10.9994 3.40356 11.2512 3.15172 11.2512 2.84106V0.591064Z"/>
                                <path
                                    d="M7.87378 0.591064C7.87378 0.280404 7.62194 0.0285645 7.31128 0.0285645C7.00062 0.0285645 6.74878 0.280404 6.74878 0.591064V2.84106C6.74878 3.15172 7.00062 3.40356 7.31128 3.40356C7.62194 3.40356 7.87378 3.15172 7.87378 2.84106V0.591064Z"/>
                                <path
                                    d="M4.49628 0.591064C4.49628 0.280404 4.24444 0.0285645 3.93378 0.0285645C3.62312 0.0285645 3.37128 0.280404 3.37128 0.591064V2.84106C3.37128 3.15172 3.62312 3.40356 3.93378 3.40356C4.24444 3.40356 4.49628 3.15172 4.49628 2.84106V0.591064Z"/>
                                <path
                                    d="M5.57379 12.859C5.57379 11.841 6.19393 11.266 6.94745 10.9237C6.31772 10.5738 5.93327 9.97518 5.93327 9.23362C5.93327 7.84346 7.14253 6.93768 9.03335 6.93768C10.665 6.93768 12.0754 7.71146 12.0754 9.2562C12.0754 10.0578 11.5991 10.5852 11.0117 10.8392C11.8151 11.133 12.4262 11.8054 12.4262 12.8442C12.4262 14.553 10.7024 15.3177 8.95704 15.3177C7.14785 15.3177 5.57379 14.5132 5.57379 12.859ZM10.4611 12.8062C10.4611 12.1583 10.0752 11.6429 8.99162 11.6429C7.89793 11.6429 7.50868 12.1281 7.50868 12.7625C7.50868 13.578 8.28429 13.9316 8.9993 13.9316C9.72377 13.9316 10.4611 13.636 10.4611 12.8062ZM7.83377 9.24273C7.83377 9.7755 8.13992 10.2237 9.04127 10.2237C9.88592 10.2237 10.171 9.82871 10.171 9.25623C10.171 8.62605 9.6497 8.29207 8.99612 8.29207C8.39034 8.29203 7.83377 8.57565 7.83377 9.24273Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown">
                            <input type="text" name="inOut" readonly value="Sep 12 - Sep 20">
                            <div class="selected-date"></div>
                        </div>
                    </div>
                    <div class="single-search-box">
                        <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M19.3024 4.186H10C9.81501 4.186 9.63755 4.1125 9.50671 3.98166C9.37587 3.85082 9.30237 3.67336 9.30237 3.48833C9.30237 3.30329 9.37587 3.12583 9.50671 2.99499C9.63755 2.86415 9.81501 2.79065 10 2.79065H19.3024C19.4874 2.79065 19.6649 2.86415 19.7957 2.99499C19.9266 3.12583 20.0001 3.30329 20.0001 3.48833C20.0001 3.67336 19.9266 3.85082 19.7957 3.98166C19.6649 4.1125 19.4874 4.186 19.3024 4.186ZM17.4419 7.90695H10C9.81501 7.90695 9.63755 7.83344 9.50671 7.7026C9.37587 7.57176 9.30237 7.39431 9.30237 7.20927C9.30237 7.02424 9.37587 6.84678 9.50671 6.71594C9.63755 6.5851 9.81501 6.5116 10 6.5116H17.4419C17.627 6.5116 17.8044 6.5851 17.9353 6.71594C18.0661 6.84678 18.1396 7.02424 18.1396 7.20927C18.1396 7.39431 18.0661 7.57176 17.9353 7.7026C17.8044 7.83344 17.627 7.90695 17.4419 7.90695ZM19.3024 13.4884H10C9.81501 13.4884 9.63755 13.4149 9.50671 13.284C9.37587 13.1532 9.30237 12.9757 9.30237 12.7907C9.30237 12.6057 9.37587 12.4282 9.50671 12.2974C9.63755 12.1665 9.81501 12.093 10 12.093H19.3024C19.4874 12.093 19.6649 12.1665 19.7957 12.2974C19.9266 12.4282 20.0001 12.6057 20.0001 12.7907C20.0001 12.9757 19.9266 13.1532 19.7957 13.284C19.6649 13.4149 19.4874 13.4884 19.3024 13.4884ZM17.4419 17.2093H10C9.81501 17.2093 9.63755 17.1358 9.50671 17.005C9.37587 16.8741 9.30237 16.6967 9.30237 16.5116C9.30237 16.3266 9.37587 16.1491 9.50671 16.0183C9.63755 15.8875 9.81501 15.814 10 15.814H17.4419C17.627 15.814 17.8044 15.8875 17.9353 16.0183C18.0661 16.1491 18.1396 16.3266 18.1396 16.5116C18.1396 16.6967 18.0661 16.8741 17.9353 17.005C17.8044 17.1358 17.627 17.2093 17.4419 17.2093Z"/>
                                <path
                                    d="M3.48826 8.83719C5.41485 8.83719 6.97665 7.27538 6.97665 5.3488C6.97665 3.42222 5.41485 1.86041 3.48826 1.86041C1.56168 1.86041 -0.00012207 3.42222 -0.00012207 5.3488C-0.00012207 7.27538 1.56168 8.83719 3.48826 8.83719Z"/>
                                <path
                                    d="M3.48826 18.1396C5.41485 18.1396 6.97665 16.5778 6.97665 14.6512C6.97665 12.7246 5.41485 11.1628 3.48826 11.1628C1.56168 11.1628 -0.00012207 12.7246 -0.00012207 14.6512C-0.00012207 16.5778 1.56168 18.1396 3.48826 18.1396Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown">
                            <input type="text" readonly value="Family Tour">
                            <span>Category</span>
                        </div>
                        <div class="custom-select-wrap two">
                            <ul class="option-list">
                                <li class="single-item">
                                    <h6>Family Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Honeymoon Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Group Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Adventure Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Solo Tour</h6>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <button type="submit" class="primary-btn1">
                        <span>
                            <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M17.7799 16.746L14.6861 13.7226L14.6137 13.6126C14.4774 13.4781 14.2936 13.4028 14.1022 13.4028C13.9107 13.4028 13.7269 13.4781 13.5906 13.6126C10.9613 16.0246 6.91095 16.1554 4.12376 13.9188C1.33658 11.6821 0.680209 7.7696 2.58814 4.77921C4.49607 1.78882 8.37732 0.64519 11.6585 2.10734C14.9396 3.56949 16.5993 7.18566 15.539 10.555C15.5016 10.675 15.4972 10.8029 15.5262 10.9251C15.5552 11.0474 15.6166 11.1597 15.7039 11.2501C15.7921 11.3421 15.9027 11.4097 16.0248 11.4463C16.1469 11.4829 16.2764 11.4872 16.4007 11.4589C16.5243 11.4317 16.6387 11.3725 16.7323 11.2872C16.8258 11.202 16.8954 11.0936 16.934 10.973C18.1996 6.97472 16.2878 2.6716 12.434 0.848041C8.58017 -0.975514 3.94271 0.225775 1.52009 3.67706C-0.902526 7.12835 -0.382565 11.7918 2.74388 14.6518C5.87033 17.5118 10.6646 17.7083 14.0273 15.1173L16.7667 17.7955C16.9042 17.9276 17.0875 18.0014 17.2782 18.0014C17.4689 18.0014 17.6522 17.9276 17.7897 17.7955C17.8568 17.7298 17.9101 17.6513 17.9465 17.5648C17.9829 17.4782 18.0016 17.3852 18.0016 17.2913C18.0016 17.1974 17.9829 17.1045 17.9465 17.0179C17.9101 16.9313 17.8568 16.8529 17.7897 16.7872L17.7799 16.746Z"/>
                                </g>
                            </svg>
                            SEARCH
                        </span>
                        <span>
                            <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M17.7799 16.746L14.6861 13.7226L14.6137 13.6126C14.4774 13.4781 14.2936 13.4028 14.1022 13.4028C13.9107 13.4028 13.7269 13.4781 13.5906 13.6126C10.9613 16.0246 6.91095 16.1554 4.12376 13.9188C1.33658 11.6821 0.680209 7.7696 2.58814 4.77921C4.49607 1.78882 8.37732 0.64519 11.6585 2.10734C14.9396 3.56949 16.5993 7.18566 15.539 10.555C15.5016 10.675 15.4972 10.8029 15.5262 10.9251C15.5552 11.0474 15.6166 11.1597 15.7039 11.2501C15.7921 11.3421 15.9027 11.4097 16.0248 11.4463C16.1469 11.4829 16.2764 11.4872 16.4007 11.4589C16.5243 11.4317 16.6387 11.3725 16.7323 11.2872C16.8258 11.202 16.8954 11.0936 16.934 10.973C18.1996 6.97472 16.2878 2.6716 12.434 0.848041C8.58017 -0.975514 3.94271 0.225775 1.52009 3.67706C-0.902526 7.12835 -0.382565 11.7918 2.74388 14.6518C5.87033 17.5118 10.6646 17.7083 14.0273 15.1173L16.7667 17.7955C16.9042 17.9276 17.0875 18.0014 17.2782 18.0014C17.4689 18.0014 17.6522 17.9276 17.7897 17.7955C17.8568 17.7298 17.9101 17.6513 17.9465 17.5648C17.9829 17.4782 18.0016 17.3852 18.0016 17.2913C18.0016 17.1974 17.9829 17.1045 17.9465 17.0179C17.9101 16.9313 17.8568 16.8529 17.7897 16.7872L17.7799 16.746Z"/>
                                </g>
                            </svg>
                            SEARCH
                        </span>
                    </button>
                </form>
                <form class="filter-input two">
                     <div class="single-search-box">
                        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M12.5944 8.99987C12.5944 10.988 10.9826 12.5998 8.99443 12.5998C7.00627 12.5998 5.39465 10.988 5.39465 8.99987C5.39465 7.0117 7.00627 5.40009 8.99443 5.40009C10.9826 5.40009 12.5944 7.0117 12.5944 8.99987Z"/>
                                <path
                                    d="M17.4601 8.4599H16.2564C15.9858 4.86535 13.1291 2.00812 9.53458 1.7372V0.539976C9.53458 0.241723 9.29268 0 8.9946 0C8.69635 0 8.45462 0.241723 8.45462 0.539976V1.7372C4.85986 2.00812 2.00297 4.86535 1.73235 8.4599H0.540018C0.241723 8.4599 0 8.7017 0 8.99987C0 9.29813 0.241723 9.53985 0.539976 9.53985H1.73239C2.00297 13.1344 4.85991 15.9916 8.45441 16.2625V17.4601C8.45441 17.7583 8.69614 18 8.99439 18C9.29251 18 9.53428 17.7583 9.53428 17.4601V16.2625C13.1289 15.9918 15.9858 13.1346 16.2564 9.53985H17.4601C17.7583 9.53985 18 9.29813 18 8.99987C18 8.70175 17.7583 8.4599 17.4601 8.4599ZM8.99443 15.2096C5.56504 15.2094 2.78509 12.4291 2.78509 8.9997C2.78522 5.57014 5.56554 2.7902 8.99494 2.7902C12.4245 2.7902 15.2046 5.57048 15.2046 8.99987C15.2005 12.428 12.4225 15.2058 8.99443 15.2096Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown destination-dropdown">
                            <input type="text" readonly value="Where are you going?">
                            <div class="input-field-value">
                                <div class="destination">
                                    <h6>Ghana</h6>
                                    <span>West Africa</span>
                                </div> 
                            </div>
                        </div>
                        <div class="custom-select-wrap">
                            <div class="custom-select-search-area">
                                <i class='bx bx-search'></i>
                                <input type="text" placeholder="Type Your Destination">
                            </div>
                            <ul class="option-list-destination">
                                <li>
                                    <div class="destination">
                                        <h6>Egypt</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Morocco</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Algeria</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Tunisia</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Libya</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Sudan</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Nigeria</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ghana</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Senegal</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ivory Coast</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mali</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Niger</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Cameroon</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Democratic Republic of the Congo</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Gabon</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Central African Republic</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Chad</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Equatorial Guinea</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Kenya</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Tanzania</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Uganda</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ethiopia</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Rwanda</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Somalia</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>South Africa</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Zimbabwe</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Botswana</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Namibia</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Zambia</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mozambique</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mauritius</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Seychelles</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Madagascar</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Comoros</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Réunion</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Maldives</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="single-search-box date-field">
                        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M16.125 1.28394H14.8913V2.43609C14.9509 2.57307 14.9755 2.72275 14.9629 2.87163C14.9502 3.0205 14.9007 3.16388 14.8188 3.28883C14.7368 3.41379 14.6251 3.51638 14.4936 3.58736C14.3622 3.65834 14.2151 3.69547 14.0657 3.6954C13.9163 3.69533 13.7692 3.65807 13.6378 3.58697C13.5064 3.51587 13.3948 3.41318 13.313 3.28815C13.2312 3.16312 13.1818 3.0197 13.1693 2.87081C13.1567 2.72193 13.1815 2.57227 13.2413 2.43534V1.28409H11.5136V2.43609C11.5733 2.57304 11.598 2.72272 11.5854 2.87159C11.5728 3.02047 11.5234 3.16388 11.4415 3.28887C11.3597 3.41386 11.248 3.5165 11.1165 3.58754C10.9851 3.65858 10.838 3.69577 10.6886 3.69577C10.5392 3.69577 10.3922 3.65858 10.2607 3.58754C10.1293 3.5165 10.0176 3.41386 9.93572 3.28887C9.85387 3.16388 9.80441 3.02047 9.79183 2.87159C9.77924 2.72272 9.80391 2.57304 9.86363 2.43609V1.28394H8.13638V2.43609C8.19609 2.57304 8.22076 2.72272 8.20818 2.87159C8.19559 3.02047 8.14613 3.16388 8.06428 3.28887C7.98242 3.41386 7.87073 3.5165 7.73929 3.58754C7.60784 3.65858 7.46079 3.69577 7.31138 3.69577C7.16197 3.69577 7.01491 3.65858 6.88346 3.58754C6.75202 3.5165 6.64033 3.41386 6.55848 3.28887C6.47662 3.16388 6.42716 3.02047 6.41457 2.87159C6.40199 2.72272 6.42666 2.57304 6.48638 2.43609V1.28394H4.75875V2.43519C4.81852 2.57212 4.84327 2.72178 4.83075 2.87066C4.81823 3.01955 4.76884 3.16297 4.68704 3.288C4.60524 3.41303 4.49359 3.51572 4.36219 3.58682C4.23078 3.65792 4.08373 3.69518 3.93432 3.69525C3.78491 3.69532 3.63784 3.65819 3.50636 3.58721C3.37489 3.51623 3.26315 3.41364 3.18124 3.28868C3.09932 3.16373 3.0498 3.02035 3.03715 2.87148C3.02449 2.7226 3.0491 2.57292 3.10875 2.43594V1.28394H1.875C1.37772 1.28394 0.900806 1.48148 0.549175 1.83311C0.197544 2.18474 0 2.66165 0 3.15894L0 16.0964C4.97191e-05 16.5937 0.19761 17.0706 0.54923 17.4222C0.90085 17.7738 1.37773 17.9714 1.875 17.9714H16.125C16.6223 17.9714 17.0992 17.7738 17.4508 17.4222C17.8024 17.0706 18 16.5937 18 16.0964V3.15894C18 2.66165 17.8025 2.18474 17.4508 1.83311C17.0992 1.48148 16.6223 1.28394 16.125 1.28394ZM17.25 15.9089C17.25 16.257 17.1117 16.5909 16.8656 16.837C16.6194 17.0832 16.2856 17.2214 15.9375 17.2214H2.0625C1.7144 17.2214 1.38056 17.0832 1.13442 16.837C0.888281 16.5909 0.75 16.257 0.75 15.9089V6.34644C0.75 5.99834 0.888281 5.6645 1.13442 5.41836C1.38056 5.17222 1.7144 5.03394 2.0625 5.03394H15.9375C16.2856 5.03394 16.6194 5.17222 16.8656 5.41836C17.1117 5.6645 17.25 5.99834 17.25 6.34644V15.9089Z"/>
                                <path
                                    d="M14.6287 0.591064C14.6287 0.280404 14.3769 0.0285645 14.0662 0.0285645C13.7556 0.0285645 13.5037 0.280404 13.5037 0.591064V2.84106C13.5037 3.15172 13.7556 3.40356 14.0662 3.40356C14.3769 3.40356 14.6287 3.15172 14.6287 2.84106V0.591064Z"/>
                                <path
                                    d="M11.2512 0.591064C11.2512 0.280404 10.9994 0.0285645 10.6887 0.0285645C10.3781 0.0285645 10.1262 0.280404 10.1262 0.591064V2.84106C10.1262 3.15172 10.3781 3.40356 10.6887 3.40356C10.9994 3.40356 11.2512 3.15172 11.2512 2.84106V0.591064Z"/>
                                <path
                                    d="M7.87378 0.591064C7.87378 0.280404 7.62194 0.0285645 7.31128 0.0285645C7.00062 0.0285645 6.74878 0.280404 6.74878 0.591064V2.84106C6.74878 3.15172 7.00062 3.40356 7.31128 3.40356C7.62194 3.40356 7.87378 3.15172 7.87378 2.84106V0.591064Z"/>
                                <path
                                    d="M4.49628 0.591064C4.49628 0.280404 4.24444 0.0285645 3.93378 0.0285645C3.62312 0.0285645 3.37128 0.280404 3.37128 0.591064V2.84106C3.37128 3.15172 3.62312 3.40356 3.93378 3.40356C4.24444 3.40356 4.49628 3.15172 4.49628 2.84106V0.591064Z"/>
                                <path
                                    d="M5.57379 12.859C5.57379 11.841 6.19393 11.266 6.94745 10.9237C6.31772 10.5738 5.93327 9.97518 5.93327 9.23362C5.93327 7.84346 7.14253 6.93768 9.03335 6.93768C10.665 6.93768 12.0754 7.71146 12.0754 9.2562C12.0754 10.0578 11.5991 10.5852 11.0117 10.8392C11.8151 11.133 12.4262 11.8054 12.4262 12.8442C12.4262 14.553 10.7024 15.3177 8.95704 15.3177C7.14785 15.3177 5.57379 14.5132 5.57379 12.859ZM10.4611 12.8062C10.4611 12.1583 10.0752 11.6429 8.99162 11.6429C7.89793 11.6429 7.50868 12.1281 7.50868 12.7625C7.50868 13.578 8.28429 13.9316 8.9993 13.9316C9.72377 13.9316 10.4611 13.636 10.4611 12.8062ZM7.83377 9.24273C7.83377 9.7755 8.13992 10.2237 9.04127 10.2237C9.88592 10.2237 10.171 9.82871 10.171 9.25623C10.171 8.62605 9.6497 8.29207 8.99612 8.29207C8.39034 8.29203 7.83377 8.57565 7.83377 9.24273Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown">
                            <input type="text" name="inOut" readonly value="Sep 12 - Sep 20">
                            <div class="selected-date"></div>
                        </div>
                    </div>
                    <div class="single-search-box">
                        <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M19.3024 4.186H10C9.81501 4.186 9.63755 4.1125 9.50671 3.98166C9.37587 3.85082 9.30237 3.67336 9.30237 3.48833C9.30237 3.30329 9.37587 3.12583 9.50671 2.99499C9.63755 2.86415 9.81501 2.79065 10 2.79065H19.3024C19.4874 2.79065 19.6649 2.86415 19.7957 2.99499C19.9266 3.12583 20.0001 3.30329 20.0001 3.48833C20.0001 3.67336 19.9266 3.85082 19.7957 3.98166C19.6649 4.1125 19.4874 4.186 19.3024 4.186ZM17.4419 7.90695H10C9.81501 7.90695 9.63755 7.83344 9.50671 7.7026C9.37587 7.57176 9.30237 7.39431 9.30237 7.20927C9.30237 7.02424 9.37587 6.84678 9.50671 6.71594C9.63755 6.5851 9.81501 6.5116 10 6.5116H17.4419C17.627 6.5116 17.8044 6.5851 17.9353 6.71594C18.0661 6.84678 18.1396 7.02424 18.1396 7.20927C18.1396 7.39431 18.0661 7.57176 17.9353 7.7026C17.8044 7.83344 17.627 7.90695 17.4419 7.90695ZM19.3024 13.4884H10C9.81501 13.4884 9.63755 13.4149 9.50671 13.284C9.37587 13.1532 9.30237 12.9757 9.30237 12.7907C9.30237 12.6057 9.37587 12.4282 9.50671 12.2974C9.63755 12.1665 9.81501 12.093 10 12.093H19.3024C19.4874 12.093 19.6649 12.1665 19.7957 12.2974C19.9266 12.4282 20.0001 12.6057 20.0001 12.7907C20.0001 12.9757 19.9266 13.1532 19.7957 13.284C19.6649 13.4149 19.4874 13.4884 19.3024 13.4884ZM17.4419 17.2093H10C9.81501 17.2093 9.63755 17.1358 9.50671 17.005C9.37587 16.8741 9.30237 16.6967 9.30237 16.5116C9.30237 16.3266 9.37587 16.1491 9.50671 16.0183C9.63755 15.8875 9.81501 15.814 10 15.814H17.4419C17.627 15.814 17.8044 15.8875 17.9353 16.0183C18.0661 16.1491 18.1396 16.3266 18.1396 16.5116C18.1396 16.6967 18.0661 16.8741 17.9353 17.005C17.8044 17.1358 17.627 17.2093 17.4419 17.2093Z"/>
                                <path
                                    d="M3.48826 8.83719C5.41485 8.83719 6.97665 7.27538 6.97665 5.3488C6.97665 3.42222 5.41485 1.86041 3.48826 1.86041C1.56168 1.86041 -0.00012207 3.42222 -0.00012207 5.3488C-0.00012207 7.27538 1.56168 8.83719 3.48826 8.83719Z"/>
                                <path
                                    d="M3.48826 18.1396C5.41485 18.1396 6.97665 16.5778 6.97665 14.6512C6.97665 12.7246 5.41485 11.1628 3.48826 11.1628C1.56168 11.1628 -0.00012207 12.7246 -0.00012207 14.6512C-0.00012207 16.5778 1.56168 18.1396 3.48826 18.1396Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown">
                            <input type="text" readonly value="Family Tour">
                            <span>Category</span>
                        </div>
                        <div class="custom-select-wrap two">
                            <ul class="option-list">
                                <li class="single-item">
                                    <h6>Family Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Honeymoon Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Group Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Adventure Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Solo Tour</h6>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <button type="submit" class="primary-btn1">
                        <span>
                            <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M17.7799 16.746L14.6861 13.7226L14.6137 13.6126C14.4774 13.4781 14.2936 13.4028 14.1022 13.4028C13.9107 13.4028 13.7269 13.4781 13.5906 13.6126C10.9613 16.0246 6.91095 16.1554 4.12376 13.9188C1.33658 11.6821 0.680209 7.7696 2.58814 4.77921C4.49607 1.78882 8.37732 0.64519 11.6585 2.10734C14.9396 3.56949 16.5993 7.18566 15.539 10.555C15.5016 10.675 15.4972 10.8029 15.5262 10.9251C15.5552 11.0474 15.6166 11.1597 15.7039 11.2501C15.7921 11.3421 15.9027 11.4097 16.0248 11.4463C16.1469 11.4829 16.2764 11.4872 16.4007 11.4589C16.5243 11.4317 16.6387 11.3725 16.7323 11.2872C16.8258 11.202 16.8954 11.0936 16.934 10.973C18.1996 6.97472 16.2878 2.6716 12.434 0.848041C8.58017 -0.975514 3.94271 0.225775 1.52009 3.67706C-0.902526 7.12835 -0.382565 11.7918 2.74388 14.6518C5.87033 17.5118 10.6646 17.7083 14.0273 15.1173L16.7667 17.7955C16.9042 17.9276 17.0875 18.0014 17.2782 18.0014C17.4689 18.0014 17.6522 17.9276 17.7897 17.7955C17.8568 17.7298 17.9101 17.6513 17.9465 17.5648C17.9829 17.4782 18.0016 17.3852 18.0016 17.2913C18.0016 17.1974 17.9829 17.1045 17.9465 17.0179C17.9101 16.9313 17.8568 16.8529 17.7897 16.7872L17.7799 16.746Z"/>
                                </g>
                            </svg>
                            SEARCH
                        </span>
                        <span>
                            <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M17.7799 16.746L14.6861 13.7226L14.6137 13.6126C14.4774 13.4781 14.2936 13.4028 14.1022 13.4028C13.9107 13.4028 13.7269 13.4781 13.5906 13.6126C10.9613 16.0246 6.91095 16.1554 4.12376 13.9188C1.33658 11.6821 0.680209 7.7696 2.58814 4.77921C4.49607 1.78882 8.37732 0.64519 11.6585 2.10734C14.9396 3.56949 16.5993 7.18566 15.539 10.555C15.5016 10.675 15.4972 10.8029 15.5262 10.9251C15.5552 11.0474 15.6166 11.1597 15.7039 11.2501C15.7921 11.3421 15.9027 11.4097 16.0248 11.4463C16.1469 11.4829 16.2764 11.4872 16.4007 11.4589C16.5243 11.4317 16.6387 11.3725 16.7323 11.2872C16.8258 11.202 16.8954 11.0936 16.934 10.973C18.1996 6.97472 16.2878 2.6716 12.434 0.848041C8.58017 -0.975514 3.94271 0.225775 1.52009 3.67706C-0.902526 7.12835 -0.382565 11.7918 2.74388 14.6518C5.87033 17.5118 10.6646 17.7083 14.0273 15.1173L16.7667 17.7955C16.9042 17.9276 17.0875 18.0014 17.2782 18.0014C17.4689 18.0014 17.6522 17.9276 17.7897 17.7955C17.8568 17.7298 17.9101 17.6513 17.9465 17.5648C17.9829 17.4782 18.0016 17.3852 18.0016 17.2913C18.0016 17.1974 17.9829 17.1045 17.9465 17.0179C17.9101 16.9313 17.8568 16.8529 17.7897 16.7872L17.7799 16.746Z"/>
                                </g>
                            </svg>
                            SEARCH
                        </span>
                    </button>
                </form>
                <form class="filter-input two">
                     <div class="single-search-box">
                        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M12.5944 8.99987C12.5944 10.988 10.9826 12.5998 8.99443 12.5998C7.00627 12.5998 5.39465 10.988 5.39465 8.99987C5.39465 7.0117 7.00627 5.40009 8.99443 5.40009C10.9826 5.40009 12.5944 7.0117 12.5944 8.99987Z"/>
                                <path
                                    d="M17.4601 8.4599H16.2564C15.9858 4.86535 13.1291 2.00812 9.53458 1.7372V0.539976C9.53458 0.241723 9.29268 0 8.9946 0C8.69635 0 8.45462 0.241723 8.45462 0.539976V1.7372C4.85986 2.00812 2.00297 4.86535 1.73235 8.4599H0.540018C0.241723 8.4599 0 8.7017 0 8.99987C0 9.29813 0.241723 9.53985 0.539976 9.53985H1.73239C2.00297 13.1344 4.85991 15.9916 8.45441 16.2625V17.4601C8.45441 17.7583 8.69614 18 8.99439 18C9.29251 18 9.53428 17.7583 9.53428 17.4601V16.2625C13.1289 15.9918 15.9858 13.1346 16.2564 9.53985H17.4601C17.7583 9.53985 18 9.29813 18 8.99987C18 8.70175 17.7583 8.4599 17.4601 8.4599ZM8.99443 15.2096C5.56504 15.2094 2.78509 12.4291 2.78509 8.9997C2.78522 5.57014 5.56554 2.7902 8.99494 2.7902C12.4245 2.7902 15.2046 5.57048 15.2046 8.99987C15.2005 12.428 12.4225 15.2058 8.99443 15.2096Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown destination-dropdown">
                            <input type="text" readonly value="Where are you going?">
                            <div class="input-field-value">
                                <div class="destination">
                                    <h6>Ghana</h6>
                                    <span>West Africa</span>
                                </div> 
                            </div>
                        </div>
                        <div class="custom-select-wrap">
                            <div class="custom-select-search-area">
                                <i class='bx bx-search'></i>
                                <input type="text" placeholder="Type Your Destination">
                            </div>
                            <ul class="option-list-destination">
                                <li>
                                    <div class="destination">
                                        <h6>Egypt</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Morocco</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Algeria</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Tunisia</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Libya</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Sudan</h6>
                                        <span>North Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Nigeria</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ghana</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Senegal</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ivory Coast</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mali</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Niger</h6>
                                        <span>West Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>50 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Cameroon</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Democratic Republic of the Congo</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Gabon</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Central African Republic</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Chad</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Equatorial Guinea</h6>
                                        <span>Central Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Kenya</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Tanzania</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Uganda</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Ethiopia</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Rwanda</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Somalia</h6>
                                        <span>East Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>30 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>South Africa</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Zimbabwe</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Botswana</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Namibia</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Zambia</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mozambique</h6>
                                        <span>Southern Africa</span>
                                    </div> 
                                    <div class="tour">
                                        <span>20 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Mauritius</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Seychelles</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Madagascar</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Comoros</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Réunion</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                                <li>
                                    <div class="destination">
                                        <h6>Maldives</h6>
                                        <span>Indian Ocean Islands</span>
                                    </div> 
                                    <div class="tour">
                                        <span>40 <br> Tours</span>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="single-search-box date-field">
                        <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M16.125 1.28394H14.8913V2.43609C14.9509 2.57307 14.9755 2.72275 14.9629 2.87163C14.9502 3.0205 14.9007 3.16388 14.8188 3.28883C14.7368 3.41379 14.6251 3.51638 14.4936 3.58736C14.3622 3.65834 14.2151 3.69547 14.0657 3.6954C13.9163 3.69533 13.7692 3.65807 13.6378 3.58697C13.5064 3.51587 13.3948 3.41318 13.313 3.28815C13.2312 3.16312 13.1818 3.0197 13.1693 2.87081C13.1567 2.72193 13.1815 2.57227 13.2413 2.43534V1.28409H11.5136V2.43609C11.5733 2.57304 11.598 2.72272 11.5854 2.87159C11.5728 3.02047 11.5234 3.16388 11.4415 3.28887C11.3597 3.41386 11.248 3.5165 11.1165 3.58754C10.9851 3.65858 10.838 3.69577 10.6886 3.69577C10.5392 3.69577 10.3922 3.65858 10.2607 3.58754C10.1293 3.5165 10.0176 3.41386 9.93572 3.28887C9.85387 3.16388 9.80441 3.02047 9.79183 2.87159C9.77924 2.72272 9.80391 2.57304 9.86363 2.43609V1.28394H8.13638V2.43609C8.19609 2.57304 8.22076 2.72272 8.20818 2.87159C8.19559 3.02047 8.14613 3.16388 8.06428 3.28887C7.98242 3.41386 7.87073 3.5165 7.73929 3.58754C7.60784 3.65858 7.46079 3.69577 7.31138 3.69577C7.16197 3.69577 7.01491 3.65858 6.88346 3.58754C6.75202 3.5165 6.64033 3.41386 6.55848 3.28887C6.47662 3.16388 6.42716 3.02047 6.41457 2.87159C6.40199 2.72272 6.42666 2.57304 6.48638 2.43609V1.28394H4.75875V2.43519C4.81852 2.57212 4.84327 2.72178 4.83075 2.87066C4.81823 3.01955 4.76884 3.16297 4.68704 3.288C4.60524 3.41303 4.49359 3.51572 4.36219 3.58682C4.23078 3.65792 4.08373 3.69518 3.93432 3.69525C3.78491 3.69532 3.63784 3.65819 3.50636 3.58721C3.37489 3.51623 3.26315 3.41364 3.18124 3.28868C3.09932 3.16373 3.0498 3.02035 3.03715 2.87148C3.02449 2.7226 3.0491 2.57292 3.10875 2.43594V1.28394H1.875C1.37772 1.28394 0.900806 1.48148 0.549175 1.83311C0.197544 2.18474 0 2.66165 0 3.15894L0 16.0964C4.97191e-05 16.5937 0.19761 17.0706 0.54923 17.4222C0.90085 17.7738 1.37773 17.9714 1.875 17.9714H16.125C16.6223 17.9714 17.0992 17.7738 17.4508 17.4222C17.8024 17.0706 18 16.5937 18 16.0964V3.15894C18 2.66165 17.8025 2.18474 17.4508 1.83311C17.0992 1.48148 16.6223 1.28394 16.125 1.28394ZM17.25 15.9089C17.25 16.257 17.1117 16.5909 16.8656 16.837C16.6194 17.0832 16.2856 17.2214 15.9375 17.2214H2.0625C1.7144 17.2214 1.38056 17.0832 1.13442 16.837C0.888281 16.5909 0.75 16.257 0.75 15.9089V6.34644C0.75 5.99834 0.888281 5.6645 1.13442 5.41836C1.38056 5.17222 1.7144 5.03394 2.0625 5.03394H15.9375C16.2856 5.03394 16.6194 5.17222 16.8656 5.41836C17.1117 5.6645 17.25 5.99834 17.25 6.34644V15.9089Z"/>
                                <path
                                    d="M14.6287 0.591064C14.6287 0.280404 14.3769 0.0285645 14.0662 0.0285645C13.7556 0.0285645 13.5037 0.280404 13.5037 0.591064V2.84106C13.5037 3.15172 13.7556 3.40356 14.0662 3.40356C14.3769 3.40356 14.6287 3.15172 14.6287 2.84106V0.591064Z"/>
                                <path
                                    d="M11.2512 0.591064C11.2512 0.280404 10.9994 0.0285645 10.6887 0.0285645C10.3781 0.0285645 10.1262 0.280404 10.1262 0.591064V2.84106C10.1262 3.15172 10.3781 3.40356 10.6887 3.40356C10.9994 3.40356 11.2512 3.15172 11.2512 2.84106V0.591064Z"/>
                                <path
                                    d="M7.87378 0.591064C7.87378 0.280404 7.62194 0.0285645 7.31128 0.0285645C7.00062 0.0285645 6.74878 0.280404 6.74878 0.591064V2.84106C6.74878 3.15172 7.00062 3.40356 7.31128 3.40356C7.62194 3.40356 7.87378 3.15172 7.87378 2.84106V0.591064Z"/>
                                <path
                                    d="M4.49628 0.591064C4.49628 0.280404 4.24444 0.0285645 3.93378 0.0285645C3.62312 0.0285645 3.37128 0.280404 3.37128 0.591064V2.84106C3.37128 3.15172 3.62312 3.40356 3.93378 3.40356C4.24444 3.40356 4.49628 3.15172 4.49628 2.84106V0.591064Z"/>
                                <path
                                    d="M5.57379 12.859C5.57379 11.841 6.19393 11.266 6.94745 10.9237C6.31772 10.5738 5.93327 9.97518 5.93327 9.23362C5.93327 7.84346 7.14253 6.93768 9.03335 6.93768C10.665 6.93768 12.0754 7.71146 12.0754 9.2562C12.0754 10.0578 11.5991 10.5852 11.0117 10.8392C11.8151 11.133 12.4262 11.8054 12.4262 12.8442C12.4262 14.553 10.7024 15.3177 8.95704 15.3177C7.14785 15.3177 5.57379 14.5132 5.57379 12.859ZM10.4611 12.8062C10.4611 12.1583 10.0752 11.6429 8.99162 11.6429C7.89793 11.6429 7.50868 12.1281 7.50868 12.7625C7.50868 13.578 8.28429 13.9316 8.9993 13.9316C9.72377 13.9316 10.4611 13.636 10.4611 12.8062ZM7.83377 9.24273C7.83377 9.7755 8.13992 10.2237 9.04127 10.2237C9.88592 10.2237 10.171 9.82871 10.171 9.25623C10.171 8.62605 9.6497 8.29207 8.99612 8.29207C8.39034 8.29203 7.83377 8.57565 7.83377 9.24273Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown">
                            <input type="text" name="inOut" readonly value="Sep 12 - Sep 20">
                            <div class="selected-date"></div>
                        </div>
                    </div>
                    <div class="single-search-box">
                        <svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <g>
                                <path
                                    d="M19.3024 4.186H10C9.81501 4.186 9.63755 4.1125 9.50671 3.98166C9.37587 3.85082 9.30237 3.67336 9.30237 3.48833C9.30237 3.30329 9.37587 3.12583 9.50671 2.99499C9.63755 2.86415 9.81501 2.79065 10 2.79065H19.3024C19.4874 2.79065 19.6649 2.86415 19.7957 2.99499C19.9266 3.12583 20.0001 3.30329 20.0001 3.48833C20.0001 3.67336 19.9266 3.85082 19.7957 3.98166C19.6649 4.1125 19.4874 4.186 19.3024 4.186ZM17.4419 7.90695H10C9.81501 7.90695 9.63755 7.83344 9.50671 7.7026C9.37587 7.57176 9.30237 7.39431 9.30237 7.20927C9.30237 7.02424 9.37587 6.84678 9.50671 6.71594C9.63755 6.5851 9.81501 6.5116 10 6.5116H17.4419C17.627 6.5116 17.8044 6.5851 17.9353 6.71594C18.0661 6.84678 18.1396 7.02424 18.1396 7.20927C18.1396 7.39431 18.0661 7.57176 17.9353 7.7026C17.8044 7.83344 17.627 7.90695 17.4419 7.90695ZM19.3024 13.4884H10C9.81501 13.4884 9.63755 13.4149 9.50671 13.284C9.37587 13.1532 9.30237 12.9757 9.30237 12.7907C9.30237 12.6057 9.37587 12.4282 9.50671 12.2974C9.63755 12.1665 9.81501 12.093 10 12.093H19.3024C19.4874 12.093 19.6649 12.1665 19.7957 12.2974C19.9266 12.4282 20.0001 12.6057 20.0001 12.7907C20.0001 12.9757 19.9266 13.1532 19.7957 13.284C19.6649 13.4149 19.4874 13.4884 19.3024 13.4884ZM17.4419 17.2093H10C9.81501 17.2093 9.63755 17.1358 9.50671 17.005C9.37587 16.8741 9.30237 16.6967 9.30237 16.5116C9.30237 16.3266 9.37587 16.1491 9.50671 16.0183C9.63755 15.8875 9.81501 15.814 10 15.814H17.4419C17.627 15.814 17.8044 15.8875 17.9353 16.0183C18.0661 16.1491 18.1396 16.3266 18.1396 16.5116C18.1396 16.6967 18.0661 16.8741 17.9353 17.005C17.8044 17.1358 17.627 17.2093 17.4419 17.2093Z"/>
                                <path
                                    d="M3.48826 8.83719C5.41485 8.83719 6.97665 7.27538 6.97665 5.3488C6.97665 3.42222 5.41485 1.86041 3.48826 1.86041C1.56168 1.86041 -0.00012207 3.42222 -0.00012207 5.3488C-0.00012207 7.27538 1.56168 8.83719 3.48826 8.83719Z"/>
                                <path
                                    d="M3.48826 18.1396C5.41485 18.1396 6.97665 16.5778 6.97665 14.6512C6.97665 12.7246 5.41485 11.1628 3.48826 11.1628C1.56168 11.1628 -0.00012207 12.7246 -0.00012207 14.6512C-0.00012207 16.5778 1.56168 18.1396 3.48826 18.1396Z"/>
                            </g>
                        </svg>
                        <div class="custom-select-dropdown">
                            <input type="text" readonly value="Family Tour">
                            <span>Category</span>
                        </div>
                        <div class="custom-select-wrap two">
                            <ul class="option-list">
                                <li class="single-item">
                                    <h6>Family Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Honeymoon Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Group Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Adventure Tour</h6>
                                </li>
                                <li class="single-item">
                                    <h6>Solo Tour</h6>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <button type="submit" class="primary-btn1">
                        <span>
                            <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M17.7799 16.746L14.6861 13.7226L14.6137 13.6126C14.4774 13.4781 14.2936 13.4028 14.1022 13.4028C13.9107 13.4028 13.7269 13.4781 13.5906 13.6126C10.9613 16.0246 6.91095 16.1554 4.12376 13.9188C1.33658 11.6821 0.680209 7.7696 2.58814 4.77921C4.49607 1.78882 8.37732 0.64519 11.6585 2.10734C14.9396 3.56949 16.5993 7.18566 15.539 10.555C15.5016 10.675 15.4972 10.8029 15.5262 10.9251C15.5552 11.0474 15.6166 11.1597 15.7039 11.2501C15.7921 11.3421 15.9027 11.4097 16.0248 11.4463C16.1469 11.4829 16.2764 11.4872 16.4007 11.4589C16.5243 11.4317 16.6387 11.3725 16.7323 11.2872C16.8258 11.202 16.8954 11.0936 16.934 10.973C18.1996 6.97472 16.2878 2.6716 12.434 0.848041C8.58017 -0.975514 3.94271 0.225775 1.52009 3.67706C-0.902526 7.12835 -0.382565 11.7918 2.74388 14.6518C5.87033 17.5118 10.6646 17.7083 14.0273 15.1173L16.7667 17.7955C16.9042 17.9276 17.0875 18.0014 17.2782 18.0014C17.4689 18.0014 17.6522 17.9276 17.7897 17.7955C17.8568 17.7298 17.9101 17.6513 17.9465 17.5648C17.9829 17.4782 18.0016 17.3852 18.0016 17.2913C18.0016 17.1974 17.9829 17.1045 17.9465 17.0179C17.9101 16.9313 17.8568 16.8529 17.7897 16.7872L17.7799 16.746Z"/>
                                </g>
                            </svg>
                            SEARCH
                        </span>
                        <span>
                            <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                <g>
                                    <path
                                        d="M17.7799 16.746L14.6861 13.7226L14.6137 13.6126C14.4774 13.4781 14.2936 13.4028 14.1022 13.4028C13.9107 13.4028 13.7269 13.4781 13.5906 13.6126C10.9613 16.0246 6.91095 16.1554 4.12376 13.9188C1.33658 11.6821 0.680209 7.7696 2.58814 4.77921C4.49607 1.78882 8.37732 0.64519 11.6585 2.10734C14.9396 3.56949 16.5993 7.18566 15.539 10.555C15.5016 10.675 15.4972 10.8029 15.5262 10.9251C15.5552 11.0474 15.6166 11.1597 15.7039 11.2501C15.7921 11.3421 15.9027 11.4097 16.0248 11.4463C16.1469 11.4829 16.2764 11.4872 16.4007 11.4589C16.5243 11.4317 16.6387 11.3725 16.7323 11.2872C16.8258 11.202 16.8954 11.0936 16.934 10.973C18.1996 6.97472 16.2878 2.6716 12.434 0.848041C8.58017 -0.975514 3.94271 0.225775 1.52009 3.67706C-0.902526 7.12835 -0.382565 11.7918 2.74388 14.6518C5.87033 17.5118 10.6646 17.7083 14.0273 15.1173L16.7667 17.7955C16.9042 17.9276 17.0875 18.0014 17.2782 18.0014C17.4689 18.0014 17.6522 17.9276 17.7897 17.7955C17.8568 17.7298 17.9101 17.6513 17.9465 17.5648C17.9829 17.4782 18.0016 17.3852 18.0016 17.2913C18.0016 17.1974 17.9829 17.1045 17.9465 17.0179C17.9101 16.9313 17.8568 16.8529 17.7897 16.7872L17.7799 16.746Z"/>
                                </g>
                            </svg>
                            SEARCH
                        </span>
                    </button>
                </form>
                
            </div>
        </div>
    </div>
    <!-- home1 Banner Section End-->

    <!-- home1 offer Section Start-->
    <div class="home1-offer-section mb-100">
        <div class="container">
            <div class="row justify-content-center mb-50 wow animate fadeInDown" data-wow-delay="200ms" data-wow-duration="1500ms">
                <div class="col-lg-6">
                    <div class="section-title text-center">
                        <h2>Discounts & Offers</h2>
                        <p>A curated list of the most popular travel packages based on different destinations.</p>
                    </div>
                </div>
            </div>
            <div class="swiper home1-offer-slider mb-40">
                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <a href="travelPackage"><img src="assets/img/home1/home1-offer-img1.jpg" alt=""></a>
                    </div>
                    <div class="swiper-slide">
                        <a href="travelPackageDetails"><img src="assets/img/home1/home1-offer-img2.jpg" alt=""></a>
                    </div>
                    <div class="swiper-slide">
                        <a href="travelPackage"><img src="assets/img/home1/home1-offer-img3.jpg" alt=""></a>
                    </div>
                    <div class="swiper-slide">
                        <a href="travelPackageDetails"><img src="assets/img/home1/home1-offer-img4.jpg" alt=""></a>
                    </div>
                    <div class="swiper-slide">
                        <a href="travelPackageDetails"><img src="assets/img/home1/home1-offer-img5.jpg" alt=""></a>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12 d-flex justify-content-center">
                    <div class="swiper-pagination1 paginations"></div>
                </div>
            </div>
        </div>
    </div>
    <!-- home1 offer Section End-->

    <!-- home1 destination Section Start-->
    <div class="home1-destination-section mb-100">
        <div class="container">
            <div class="row justify-content-center mb-60 wow animate fadeInDown" data-wow-delay="200ms" data-wow-duration="1500ms">
                <div class="col-lg-10">
                    <div class="section-title text-center">
                        <h2>Featured Destinations</h2>
                    </div>
                    <ul class="nav nav-pills" id="pills-tab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="pills-northafrica-tab" data-bs-toggle="pill" data-bs-target="#pills-northafrica" type="button" role="tab" aria-controls="pills-northafrica" aria-selected="true">North Africa</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="pills-westafrica-tab" data-bs-toggle="pill" data-bs-target="#pills-westafrica" type="button" role="tab" aria-controls="pills-westafrica" aria-selected="false">West Africa</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="pills-centralafrica-tab" data-bs-toggle="pill" data-bs-target="#pills-centralafrica" type="button" role="tab" aria-controls="pills-centralafrica" aria-selected="false">Central Africa</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="pills-eastafrica-tab" data-bs-toggle="pill" data-bs-target="#pills-eastafrica" type="button" role="tab" aria-controls="pills-eastafrica" aria-selected="false">East Africa</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="pills-southernafrica-tab" data-bs-toggle="pill" data-bs-target="#pills-southernafrica" type="button" role="tab" aria-controls="pills-southernafrica" aria-selected="false">Southern Africa</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="pills-oceania-tab" data-bs-toggle="pill" data-bs-target="#pills-oceania" type="button" role="tab" aria-controls="pills-oceania" aria-selected="false">Indian Ocean Islands</button>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-northafrica" role="tabpanel" aria-labelledby="pills-northafrica-tab">
                    <div class="swiper home1-destination-slider mb-40">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img1.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Cairo, Egypt
                                        </a>
                                        <div class="content">
                                            <p>140 tours | 5 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img2.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Marrakech, Morocco
                                        </a>
                                        <div class="content">
                                            <p>160 tours | 30 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img3.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Algiers, Algeria
                                        </a>
                                        <div class="content">
                                            <p>100 tours | 15 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img4.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Tunis, Tunisia
                                        </a>
                                        <div class="content">
                                            <p>110 tours | 26 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img5.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Tripoli, Libya
                                        </a>
                                        <div class="content">
                                            <p>70 tours | 23 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img6.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Khartoum, Sudan
                                        </a>
                                        <div class="content">
                                            <p>50 tours | 17 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider-pagi-wrap">
                        <div class="home1-destination-pagi paginations"></div>
                    </div>
                </div>
                <div class="tab-pane fade" id="pills-westafrica" role="tabpanel" aria-labelledby="pills-westafrica-tab">
                    <div class="swiper home1-destination-slider mb-40">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img1.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Accra, Ghana
                                        </a>
                                        <div class="content">
                                            <p>85 tours | 30 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img2.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Lagos, Nigeria
                                        </a>
                                        <div class="content">
                                            <p>120 tours | 45 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img3.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Dakar, Senegal
                                        </a>
                                        <div class="content">
                                            <p>65 tours | 25 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img4.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Abidjan, Ivory Coast
                                        </a>
                                        <div class="content">
                                            <p>90 tours | 35 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img5.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Bamako, Mali
                                        </a>
                                        <div class="content">
                                            <p>50 tours | 18 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img6.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Freetown, Sierra Leone
                                        </a>
                                        <div class="content">
                                            <p>55 tours | 20 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider-pagi-wrap">
                        <div class="home1-destination-pagi paginations"></div>
                    </div>
                </div>
                <div class="tab-pane fade" id="pills-centralafrica" role="tabpanel" aria-labelledby="pills-centralafrica-tab">
                    <div class="swiper home1-destination-slider mb-40">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img1.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Yaoundé, Cameroon
                                        </a>
                                        <div class="content">
                                            <p>80 tours | 28 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img2.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Kinshasa, Democratic Republic of the Congo
                                        </a>
                                        <div class="content">
                                            <p>100 tours | 40 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img3.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Libreville, Gabon
                                        </a>
                                        <div class="content">
                                            <p>60 tours | 22 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img4.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Bangui, Central African Republic
                                        </a>
                                        <div class="content">
                                            <p>45 tours | 15 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img5.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            N'Djamena, Chad
                                        </a>
                                        <div class="content">
                                            <p>50 tours | 18 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img6.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Brazzaville, Republic of the Congo
                                        </a>
                                        <div class="content">
                                            <p>55 tours | 20 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider-pagi-wrap">
                        <div class="home1-destination-pagi paginations"></div>
                    </div>
                </div>
                <div class="tab-pane fade" id="pills-eastafrica" role="tabpanel" aria-labelledby="pills-eastafrica-tab">
                    <div class="swiper home1-destination-slider mb-40">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img1.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Nairobi, Kenya
                                        </a>
                                        <div class="content">
                                            <p>90 tours | 35 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img2.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Dar es Salaam, Tanzania
                                        </a>
                                        <div class="content">
                                            <p>85 tours | 30 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img3.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Kampala, Uganda
                                        </a>
                                        <div class="content">
                                            <p>70 tours | 25 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img4.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Addis Ababa, Ethiopia
                                        </a>
                                        <div class="content">
                                            <p>65 tours | 22 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img5.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Kigali, Rwanda
                                        </a>
                                        <div class="content">
                                            <p>60 tours | 20 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img6.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Mogadishu, Somalia
                                        </a>
                                        <div class="content">
                                            <p>50 tours | 15 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider-pagi-wrap">
                        <div class="home1-destination-pagi paginations"></div>
                    </div>
                </div>
                <div class="tab-pane fade" id="pills-southernafrica" role="tabpanel" aria-labelledby="pills-southernafrica-tab">
                    <div class="swiper home1-destination-slider mb-40">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img1.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Cape Town, South Africa
                                        </a>
                                        <div class="content">
                                            <p>100 tours | 40 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img2.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Windhoek, Namibia
                                        </a>
                                        <div class="content">
                                            <p>75 tours | 25 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img3.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Gaborone, Botswana
                                        </a>
                                        <div class="content">
                                            <p>60 tours | 20 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img4.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Harare, Zimbabwe
                                        </a>
                                        <div class="content">
                                            <p>55 tours | 18 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img5.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Lusaka, Zambia
                                        </a>
                                        <div class="content">
                                            <p>50 tours | 15 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img6.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Maputo, Mozambique
                                        </a>
                                        <div class="content">
                                            <p>65 tours | 22 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider-pagi-wrap">
                        <div class="home1-destination-pagi paginations"></div>
                    </div>
                </div>
                <div class="tab-pane fade" id="pills-oceania" role="tabpanel" aria-labelledby="pills-oceania-tab">
                    <div class="swiper home1-destination-slider mb-40">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img1.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Port Louis, Mauritius
                                        </a>
                                        <div class="content">
                                            <p>80 tours | 30 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img2.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Victoria, Seychelles
                                        </a>
                                        <div class="content">
                                            <p>75 tours | 28 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img3.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Antananarivo, Madagascar
                                        </a>
                                        <div class="content">
                                            <p>60 tours | 20 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img4.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Moroni, Comoros
                                        </a>
                                        <div class="content">
                                            <p>50 tours | 15 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img5.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Malé, Maldives
                                        </a>
                                        <div class="content">
                                            <p>85 tours | 35 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="destination-card">
                                    <a href="destinationDetails" class="destination-img">
                                        <img src="assets/img/home1/destination-img6.jpg" alt="">
                                    </a>
                                    <div class="destination-content">
                                        <a href="destinationDetails" class="title-area">
                                            <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M7.81276 0C4.31734 0 1.47305 2.84433 1.47305 6.34163C1.47305 9.07242 5.2847 13.5258 6.92356 15.3136C7.15052 15.5628 7.47606 15.7042 7.81276 15.7042C8.14946 15.7042 8.475 15.5628 8.70196 15.3136C10.3408 13.5258 14.1525 9.07238 14.1525 6.34163C14.1525 2.84433 11.3082 0 7.81276 0ZM8.35966 14.9991C8.21642 15.1535 8.02297 15.2391 7.81276 15.2391C7.60255 15.2391 7.4091 15.1536 7.26586 14.9991C5.66417 13.2525 1.93812 8.90875 1.93812 6.34167C1.93812 3.10103 4.57221 0.465067 7.81276 0.465067C11.0533 0.465067 13.6874 3.10103 13.6874 6.34167C13.6874 8.90875 9.96135 13.2524 8.35966 14.9991Z"/>
                                                <path
                                                    d="M7.81277 9.76634C9.6713 9.76634 11.1779 8.25971 11.1779 6.40118C11.1779 4.54265 9.6713 3.03601 7.81277 3.03601C5.95424 3.03601 4.4476 4.54265 4.4476 6.40118C4.4476 8.25971 5.95424 9.76634 7.81277 9.76634Z"/>
                                            </svg>
                                            Saint-Denis, Réunion
                                        </a>
                                        <div class="content">
                                            <p>65 tours | 22 hotels</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="slider-pagi-wrap">
                        <div class="home1-destination-pagi paginations"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- home1 detination Section End-->

    <!-- home1 service Section Start-->
    <style>
        .service-list {
            display: grid !important;
            grid-template-columns: repeat(3, 1fr) !important;
            gap: 20px !important;
            list-style: none;
            padding: 0;
        }
        .single-service {
            display: flex;
            align-items: center;
            background: #f8f8f8;
            padding: 20px;
            border-radius: 8px;
            transition: transform 0.3s ease;
        }
        .single-service:hover {
            transform: translateY(-5px);
        }
        .icon {
            margin-right: 15px;
        }
        .content h4 {
            margin: 0 0 10px;
            font-size: 1.2rem;
        }
        .content p {
            margin: 0;
            font-size: 0.9rem;
            color: #555;
        }
        @media (max-width: 992px) {
            .service-list {
                grid-template-columns: repeat(2, 1fr);
            }
        }
        @media (max-width: 576px) {
            .service-list {
                grid-template-columns: 1fr;
            }
        }
    </style>
    <div class="home1-service-section mb-100">
        <div class="container">
            <div class="service-wrapper">
                <div class="row justify-content-center wow animate fadeInDown" data-wow-delay="200ms" data-wow-duration="1500ms">
                    <div class="col-lg-9">
                        <div class="section-title">
                            <h2>Discover Our Premier Travel Services</h2>
                            <p>Elevate your journey with our expertly curated services, designed for seamless and unforgettable travel experiences.</p>
                            <svg height="6" viewBox="0 0 872 6" xmlns="http://www.w3.org/2000/svg">
                                <path d="M5 2.5L0 0.113249V5.88675L5 3.5V2.5ZM867 3.5L872 5.88675V0.113249L867 2.5V3.5ZM4.5 3.5H867.5V2.5H4.5V3.5Z" />
                            </svg>
                        </div>
                    </div>
                </div>
                <ul class="service-list wow animate fadeInUp" data-wow-delay="200ms" data-wow-duration="1500ms">
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M15 0C21.4662 0 26.7081 5.24194 26.7081 11.7081C26.7081 18.1743 21.4662 23.4163 15 23.4163C8.53375 23.4163 3.29187 18.1743 3.29187 11.7081C3.29187 5.24194 8.53375 0 15 0ZM23.7899 20.886C22.1639 22.4438 20.1246 23.5736 17.8531 24.0947L19.8539 28.7401L22.0141 25.2591L26.0274 26.081L23.7899 20.886ZM12.758 24.2191C10.4637 23.8108 8.38243 22.7867 6.69112 21.324L3.97262 27.1631L8.00831 26.4579L10.0666 30L12.758 24.2191ZM15.1981 7.58544C15.1294 7.40431 14.8707 7.40431 14.8019 7.58544L13.7848 10.2644C13.7706 10.3033 13.7451 10.337 13.7117 10.3613C13.6782 10.3856 13.6383 10.3994 13.597 10.4009L10.7348 10.5404C10.5401 10.5498 10.4604 10.7951 10.6124 10.9172L12.8459 12.7124C12.8785 12.7379 12.9027 12.7726 12.9154 12.8119C12.9282 12.8512 12.929 12.8934 12.9177 12.9332L12.1659 15.6984C12.1147 15.8865 12.3233 16.0381 12.4864 15.9313L14.8839 14.3618C14.9182 14.3387 14.9586 14.3264 15 14.3264C15.0413 14.3264 15.0817 14.3387 15.116 14.3618L17.5136 15.9313C17.6767 16.0381 17.8853 15.8865 17.8341 15.6984L17.0823 12.9333C17.071 12.8935 17.0717 12.8513 17.0845 12.8119C17.0973 12.7726 17.1215 12.738 17.154 12.7125L19.3876 10.9172C19.5395 10.7951 19.4598 10.5499 19.2652 10.5404L16.403 10.4009C16.3617 10.3994 16.3218 10.3856 16.2883 10.3613C16.2549 10.337 16.2294 10.3033 16.2152 10.2645L15.1981 7.58544ZM16.1065 3.43C15.4404 2.93137 14.5596 2.93137 13.8935 3.43C12.1593 4.72812 12.5526 4.58494 10.3897 4.70525C9.55893 4.75144 8.88418 5.31763 8.69443 6.12775C8.20037 8.23687 8.40962 7.87438 6.83012 9.35688C6.22343 9.92631 6.0705 10.7937 6.44587 11.5363C7.42312 13.4695 7.35043 13.0574 7.09337 15.2083C6.99462 16.0344 7.43506 16.7972 8.19987 17.1248C10.1912 17.9776 9.87056 17.7085 11.0562 19.5215C11.5116 20.2179 12.3392 20.5191 13.1357 20.2784C15.2093 19.6517 14.7908 19.6517 16.8644 20.2784C17.6609 20.5191 18.4886 20.2178 18.9439 19.5215C20.1296 17.7086 19.809 17.9776 21.8002 17.1248C22.5651 16.7972 23.0055 16.0344 22.9067 15.2083C22.6497 13.0573 22.577 13.4695 23.5542 11.5363C23.9296 10.7937 23.7766 9.92631 23.17 9.35688C21.5904 7.87444 21.7997 8.23687 21.3057 6.12775C21.1159 5.31763 20.4412 4.7515 19.6104 4.70525C17.4474 4.58494 17.8407 4.72806 16.1065 3.43Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>Guided Tours</h4>
                            <p>Explore with expert-led tours tailored to your interests, ensuring an immersive and unforgettable journey.</p>
                        </div>
                    </li>
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M20.4896 4.14404C19.8384 3.81275 19.0419 4.07233 18.7107 4.72379L8.87091 24.0772C8.53971 24.7285 8.79929 25.5249 9.45066 25.8561C9.63601 25.9507 9.84112 26 10.0492 26C10.5313 26 10.996 25.7356 11.2296 25.2763L21.0694 5.92297C21.4006 5.27161 21.141 4.47523 20.4896 4.14404ZM13.4243 10.5172C13.4243 7.75042 11.1733 5.49943 8.40653 5.49943C5.63991 5.49943 3.38892 7.75042 3.38892 10.5172C3.38892 13.2839 5.63991 15.5349 8.40662 15.5349C11.1734 15.5349 13.4243 13.2839 13.4243 10.5172ZM8.40662 12.8889C7.09894 12.8889 6.03496 11.8249 6.03496 10.5172C6.03496 9.20945 7.09894 8.14548 8.40662 8.14548C9.71438 8.14548 10.7784 9.20937 10.7784 10.5172C10.7783 11.8249 9.71438 12.8889 8.40662 12.8889ZM21.5934 14.4653C18.8266 14.4653 16.5756 16.7163 16.5756 19.483C16.5756 22.2497 18.8265 24.5007 21.5934 24.5007C24.3601 24.5007 26.6111 22.2498 26.6111 19.483C26.6111 16.7162 24.3602 14.4653 21.5934 14.4653ZM21.5934 21.8546C20.2856 21.8546 19.2216 20.7906 19.2216 19.4829C19.2216 18.1752 20.2856 17.1113 21.5934 17.1113C22.901 17.1113 23.965 18.1752 23.965 19.4829C23.965 20.7906 22.9011 21.8546 21.5934 21.8546Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>Car Rental</h4>
                            <p>Enjoy flexible and reliable car rental services for seamless travel at your own pace.</p>
                        </div>
                    </li>
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M27.6977 12.3954C27.6977 11.446 27.32 10.5343 26.6493 9.86234C25.9771 9.19137 25.0661 8.81436 24.1163 8.81396H5.88376C4.93437 8.81396 4.02274 9.19164 3.35074 9.86234C2.67978 10.5346 2.30277 11.4455 2.30237 12.3954V25.4186C2.30237 26.368 2.68004 27.2796 3.35074 27.9516C4.02303 28.6226 4.93394 28.9996 5.88376 29H24.1163C25.0657 29 25.9773 28.6223 26.6493 27.9516C27.3203 27.2794 27.6973 26.3684 27.6977 25.4186V12.3954ZM25.7442 12.3954V25.4186C25.7435 25.8502 25.5718 26.2638 25.2667 26.569C24.9615 26.8741 24.5479 27.0458 24.1163 27.0465H5.88376C5.45223 27.0458 5.03856 26.8741 4.73342 26.569C4.42828 26.2638 4.25655 25.8502 4.25586 25.4186V12.3954C4.25655 11.9638 4.42828 11.5502 4.73342 11.245C5.03856 10.9399 5.45223 10.7681 5.88376 10.7675H24.1163C24.5479 10.7681 24.9615 10.9399 25.2667 11.245C25.5718 11.5502 25.7435 11.9638 25.7442 12.3954Z"/>
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M15.5497 5.9527L16.7986 3.27511L23.2907 6.30172L21.8555 9.37781C21.746 9.61268 21.7343 9.88143 21.823 10.1249C21.9116 10.3684 22.0934 10.5667 22.3283 10.6762C22.5631 10.7857 22.8319 10.7974 23.0754 10.7088C23.3189 10.6201 23.5172 10.4384 23.6267 10.2035L25.4734 6.24181C25.5826 6.00725 25.5944 5.73893 25.506 5.49573C25.4176 5.25254 25.2363 5.05436 25.0019 4.94469L16.7387 1.09111C16.504 0.98194 16.2355 0.970407 15.9923 1.05905C15.7491 1.14769 15.551 1.32926 15.4416 1.56386L13.7798 5.12832C13.7206 5.24516 13.6856 5.37271 13.6767 5.50338C13.6678 5.63405 13.6853 5.76517 13.7281 5.88894C13.7709 6.01271 13.8382 6.12661 13.9259 6.22386C14.0137 6.32111 14.1201 6.39972 14.2388 6.45502C14.3575 6.51031 14.4861 6.54117 14.617 6.54575C14.7479 6.55033 14.8784 6.52855 15.0007 6.48168C15.123 6.43482 15.2346 6.36384 15.3289 6.27296C15.4232 6.18209 15.4983 6.07317 15.5497 5.9527Z"/>
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M7.66406 10.2035L10.2869 4.57748L16.779 7.60409L15.9521 9.37785C15.8494 9.61152 15.8424 9.87607 15.9323 10.1149C16.0223 10.3537 16.2022 10.5478 16.4334 10.6557C16.6647 10.7636 16.929 10.7767 17.1698 10.6922C17.4106 10.6077 17.6088 10.4323 17.7219 10.2035L18.9617 7.54418C19.071 7.30962 19.0827 7.04129 18.9943 6.7981C18.9059 6.55491 18.7247 6.35673 18.4903 6.24706L10.227 2.39348C9.99231 2.28431 9.72386 2.27278 9.48064 2.36142C9.23742 2.45006 9.03934 2.63163 8.92992 2.86623L5.89289 9.37785C5.7834 9.61273 5.7717 9.88147 5.86035 10.125C5.94901 10.3685 6.13077 10.5668 6.36564 10.6763C6.60051 10.7858 6.86925 10.7975 7.11275 10.7088C7.35626 10.6202 7.55456 10.4384 7.66406 10.2035ZM27.6977 15C27.6977 14.7409 27.5948 14.4925 27.4116 14.3093C27.2285 14.1262 26.98 14.0232 26.721 14.0232H21.5104C20.5605 14.0232 19.6496 14.4006 18.978 15.0722C18.3063 15.7439 17.929 16.6548 17.929 17.6046V20.2093C17.929 21.1591 18.3063 22.0701 18.978 22.7417C19.6496 23.4134 20.5605 23.7907 21.5104 23.7907H26.721C26.98 23.7907 27.2285 23.6878 27.4116 23.5046C27.5948 23.3214 27.6977 23.073 27.6977 22.8139V15ZM25.7442 15.9767V21.8372H21.5104C20.6118 21.8372 19.8825 21.1079 19.8825 20.2093V17.6046C19.8825 16.706 20.6118 15.9767 21.5104 15.9767H25.7442Z"/>
                                <path
                                    d="M22.1628 20.2094C22.8821 20.2094 23.4651 19.6263 23.4651 18.9071C23.4651 18.1878 22.8821 17.6047 22.1628 17.6047C21.4435 17.6047 20.8605 18.1878 20.8605 18.9071C20.8605 19.6263 21.4435 20.2094 22.1628 20.2094Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>Ticketing</h4>
                            <p>Secure the best flight deals with our streamlined ticketing services, tailored to your schedule.</p>
                        </div>
                    </li>
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M15 0C21.4662 0 26.7081 5.24194 26.7081 11.7081C26.7081 18.1743 21.4662 23.4163 15 23.4163C8.53375 23.4163 3.29187 18.1743 3.29187 11.7081C3.29187 5.24194 8.53375 0 15 0ZM23.7899 20.886C22.1639 22.4438 20.1246 23.5736 17.8531 24.0947L19.8539 28.7401L22.0141 25.2591L26.0274 26.081L23.7899 20.886ZM12.758 24.2191C10.4637 23.8108 8.38243 22.7867 6.69112 21.324L3.97262 27.1631L8.00831 26.4579L10.0666 30L12.758 24.2191ZM15.1981 7.58544C15.1294 7.40431 14.8707 7.40431 14.8019 7.58544L13.7848 10.2644C13.7706 10.3033 13.7451 10.337 13.7117 10.3613C13.6782 10.3856 13.6383 10.3994 13.597 10.4009L10.7348 10.5404C10.5401 10.5498 10.4604 10.7951 10.6124 10.9172L12.8459 12.7124C12.8785 12.7379 12.9027 12.7726 12.9154 12.8119C12.9282 12.8512 12.929 12.8934 12.9177 12.9332L12.1659 15.6984C12.1147 15.8865 12.3233 16.0381 12.4864 15.9313L14.8839 14.3618C14.9182 14.3387 14.9586 14.3264 15 14.3264C15.0413 14.3264 15.0817 14.3387 15.116 14.3618L17.5136 15.9313C17.6767 16.0381 17.8853 15.8865 17.8341 15.6984L17.0823 12.9333C17.071 12.8935 17.0717 12.8513 17.0845 12.8119C17.0973 12.7726 17.1215 12.738 17.154 12.7125L19.3876 10.9172C19.5395 10.7951 19.4598 10.5499 19.2652 10.5404L16.403 10.4009C16.3617 10.3994 16.3218 10.3856 16.2883 10.3613C16.2549 10.337 16.2294 10.3033 16.2152 10.2645L15.1981 7.58544ZM16.1065 3.43C15.4404 2.93137 14.5596 2.93137 13.8935 3.43C12.1593 4.72812 12.5526 4.58494 10.3897 4.70525C9.55893 4.75144 8.88418 5.31763 8.69443 6.12775C8.20037 8.23687 8.40962 7.87438 6.83012 9.35688C6.22343 9.92631 6.0705 10.7937 6.44587 11.5363C7.42312 13.4695 7.35043 13.0574 7.09337 15.2083C6.99462 16.0344 7.43506 16.7972 8.19987 17.1248C10.1912 17.9776 9.87056 17.7085 11.0562 19.5215C11.5116 20.2179 12.3392 20.5191 13.1357 20.2784C15.2093 19.6517 14.7908 19.6517 16.8644 20.2784C17.6609 20.5191 18.4886 20.2178 18.9439 19.5215C20.1296 17.7086 19.809 17.9776 21.8002 17.1248C22.5651 16.7972 23.0055 16.0344 22.9067 15.2083C22.6497 13.0573 22.577 13.4695 23.5542 11.5363C23.9296 10.7937 23.7766 9.92631 23.17 9.35688C21.5904 7.87444 21.7997 8.23687 21.3057 6.12775C21.1159 5.31763 20.4412 4.7515 19.6104 4.70525C17.4474 4.58494 17.8407 4.72806 16.1065 3.43Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>MICE (Meetings, Incentives, Conferences, Events)</h4>
                            <p>Plan and execute flawless corporate events, conferences, and incentive trips with our professional MICE services.</p>
                        </div>
                    </li>
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M20.4896 4.14404C19.8384 3.81275 19.0419 4.07233 18.7107 4.72379L8.87091 24.0772C8.53971 24.7285 8.79929 25.5249 9.45066 25.8561C9.63601 25.9507 9.84112 26 10.0492 26C10.5313 26 10.996 25.7356 11.2296 25.2763L21.0694 5.92297C21.4006 5.27161 21.141 4.47523 20.4896 4.14404ZM13.4243 10.5172C13.4243 7.75042 11.1733 5.49943 8.40653 5.49943C5.63991 5.49943 3.38892 7.75042 3.38892 10.5172C3.38892 13.2839 5.63991 15.5349 8.40662 15.5349C11.1734 15.5349 13.4243 13.2839 13.4243 10.5172ZM8.40662 12.8889C7.09894 12.8889 6.03496 11.8249 6.03496 10.5172C6.03496 9.20945 7.09894 8.14548 8.40662 8.14548C9.71438 8.14548 10.7784 9.20937 10.7784 10.5172C10.7783 11.8249 9.71438 12.8889 8.40662 12.8889ZM21.5934 14.4653C18.8266 14.4653 16.5756 16.7163 16.5756 19.483C16.5756 22.2497 18.8265 24.5007 21.5934 24.5007C24.3601 24.5007 26.6111 22.2498 26.6111 19.483C26.6111 16.7162 24.3602 14.4653 21.5934 14.4653ZM21.5934 21.8546C20.2856 21.8546 19.2216 20.7906 19.2216 19.4829C19.2216 18.1752 20.2856 17.1113 21.5934 17.1113C22.901 17.1113 23.965 18.1752 23.965 19.4829C23.965 20.7906 22.9011 21.8546 21.5934 21.8546Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>Hotel Reservations</h4>
                            <p>Book premium accommodations with exclusive deals, ensuring comfort and convenience during your stay.</p>
                        </div>
                    </li>
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M27.6977 12.3954C27.6977 11.446 27.32 10.5343 26.6493 9.86234C25.9771 9.19137 25.0661 8.81436 24.1163 8.81396H5.88376C4.93437 8.81396 4.02274 9.19164 3.35074 9.86234C2.67978 10.5346 2.30277 11.4455 2.30237 12.3954V25.4186C2.30237 26.368 2.68004 27.2796 3.35074 27.9516C4.02303 28.6226 4.93394 28.9996 5.88376 29H24.1163C25.0657 29 25.9773 28.6223 26.6493 27.9516C27.3203 27.2794 27.6973 26.3684 27.6977 25.4186V12.3954ZM25.7442 12.3954V25.4186C25.7435 25.8502 25.5718 26.2638 25.2667 26.569C24.9615 26.8741 24.5479 27.0458 24.1163 27.0465H5.88376C5.45223 27.0458 5.03856 26.8741 4.73342 26.569C4.42828 26.2638 4.25655 25.8502 4.25586 25.4186V12.3954C4.25655 11.9638 4.42828 11.5502 4.73342 11.245C5.03856 10.9399 5.45223 10.7681 5.88376 10.7675H24.1163C24.5479 10.7681 24.9615 10.9399 25.2667 11.245C25.5718 11.5502 25.7435 11.9638 25.7442 12.3954Z"/>
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M15.5497 5.9527L16.7986 3.27511L23.2907 6.30172L21.8555 9.37781C21.746 9.61268 21.7343 9.88143 21.823 10.1249C21.9116 10.3684 22.0934 10.5667 22.3283 10.6762C22.5631 10.7857 22.8319 10.7974 23.0754 10.7088C23.3189 10.6201 23.5172 10.4384 23.6267 10.2035L25.4734 6.24181C25.5826 6.00725 25.5944 5.73893 25.506 5.49573C25.4176 5.25254 25.2363 5.05436 25.0019 4.94469L16.7387 1.09111C16.504 0.98194 16.2355 0.970407 15.9923 1.05905C15.7491 1.14769 15.551 1.32926 15.4416 1.56386L13.7798 5.12832C13.7206 5.24516 13.6856 5.37271 13.6767 5.50338C13.6678 5.63405 13.6853 5.76517 13.7281 5.88894C13.7709 6.01271 13.8382 6.12661 13.9259 6.22386C14.0137 6.32111 14.1201 6.39972 14.2388 6.45502C14.3575 6.51031 14.4861 6.54117 14.617 6.54575C14.7479 6.55033 14.8784 6.52855 15.0007 6.48168C15.123 6.43482 15.2346 6.36384 15.3289 6.27296C15.4232 6.18209 15.4983 6.07317 15.5497 5.9527Z"/>
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M7.66406 10.2035L10.2869 4.57748L16.779 7.60409L15.9521 9.37785C15.8494 9.61152 15.8424 9.87607 15.9323 10.1149C16.0223 10.3537 16.2022 10.5478 16.4334 10.6557C16.6647 10.7636 16.929 10.7767 17.1698 10.6922C17.4106 10.6077 17.6088 10.4323 17.7219 10.2035L18.9617 7.54418C19.071 7.30962 19.0827 7.04129 18.9943 6.7981C18.9059 6.55491 18.7247 6.35673 18.4903 6.24706L10.227 2.39348C9.99231 2.28431 9.72386 2.27278 9.48064 2.36142C9.23742 2.45006 9.03934 2.63163 8.92992 2.86623L5.89289 9.37785C5.7834 9.61273 5.7717 9.88147 5.86035 10.125C5.94901 10.3685 6.13077 10.5668 6.36564 10.6763C6.60051 10.7858 6.86925 10.7975 7.11275 10.7088C7.35626 10.6202 7.55456 10.4384 7.66406 10.2035ZM27.6977 15C27.6977 14.7409 27.5948 14.4925 27.4116 14.3093C27.2285 14.1262 26.98 14.0232 26.721 14.0232H21.5104C20.5605 14.0232 19.6496 14.4006 18.978 15.0722C18.3063 15.7439 17.929 16.6548 17.929 17.6046V20.2093C17.929 21.1591 18.3063 22.0701 18.978 22.7417C19.6496 23.4134 20.5605 23.7907 21.5104 23.7907H26.721C26.98 23.7907 27.2285 23.6878 27.4116 23.5046C27.5948 23.3214 27.6977 23.073 27.6977 22.8139V15ZM25.7442 15.9767V21.8372H21.5104C20.6118 21.8372 19.8825 21.1079 19.8825 20.2093V17.6046C19.8825 16.706 20.6118 15.9767 21.5104 15.9767H25.7442Z"/>
                                <path
                                    d="M22.1628 20.2094C22.8821 20.2094 23.4651 19.6263 23.4651 18.9071C23.4651 18.1878 22.8821 17.6047 22.1628 17.6047C21.4435 17.6047 20.8605 18.1878 20.8605 18.9071C20.8605 19.6263 21.4435 20.2094 22.1628 20.2094Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>Visa Assistance</h4>
                            <p>Simplify your travel with expert visa support, ensuring a hassle-free application process.</p>
                        </div>
                    </li>
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M15 0C21.4662 0 26.7081 5.24194 26.7081 11.7081C26.7081 18.1743 21.4662 23.4163 15 23.4163C8.53375 23.4163 3.29187 18.1743 3.29187 11.7081C3.29187 5.24194 8.53375 0 15 0ZM23.7899 20.886C22.1639 22.4438 20.1246 23.5736 17.8531 24.0947L19.8539 28.7401L22.0141 25.2591L26.0274 26.081L23.7899 20.886ZM12.758 24.2191C10.4637 23.8108 8.38243 22.7867 6.69112 21.324L3.97262 27.1631L8.00831 26.4579L10.0666 30L12.758 24.2191ZM15.1981 7.58544C15.1294 7.40431 14.8707 7.40431 14.8019 7.58544L13.7848 10.2644C13.7706 10.3033 13.7451 10.337 13.7117 10.3613C13.6782 10.3856 13.6383 10.3994 13.597 10.4009L10.7348 10.5404C10.5401 10.5498 10.4604 10.7951 10.6124 10.9172L12.8459 12.7124C12.8785 12.7379 12.9027 12.7726 12.9154 12.8119C12.9282 12.8512 12.929 12.8934 12.9177 12.9332L12.1659 15.6984C12.1147 15.8865 12.3233 16.0381 12.4864 15.9313L14.8839 14.3618C14.9182 14.3387 14.9586 14.3264 15 14.3264C15.0413 14.3264 15.0817 14.3387 15.116 14.3618L17.5136 15.9313C17.6767 16.0381 17.8853 15.8865 17.8341 15.6984L17.0823 12.9333C17.071 12.8935 17.0717 12.8513 17.0845 12.8119C17.0973 12.7726 17.1215 12.738 17.154 12.7125L19.3876 10.9172C19.5395 10.7951 19.4598 10.5499 19.2652 10.5404L16.403 10.4009C16.3617 10.3994 16.3218 10.3856 16.2883 10.3613C16.2549 10.337 16.2294 10.3033 16.2152 10.2645L15.1981 7.58544ZM16.1065 3.43C15.4404 2.93137 14.5596 2.93137 13.8935 3.43C12.1593 4.72812 12.5526 4.58494 10.3897 4.70525C9.55893 4.75144 8.88418 5.31763 8.69443 6.12775C8.20037 8.23687 8.40962 7.87438 6.83012 9.35688C6.22343 9.92631 6.0705 10.7937 6.44587 11.5363C7.42312 13.4695 7.35043 13.0574 7.09337 15.2083C6.99462 16.0344 7.43506 16.7972 8.19987 17.1248C10.1912 17.9776 9.87056 17.7085 11.0562 19.5215C11.5116 20.2179 12.3392 20.5191 13.1357 20.2784C15.2093 19.6517 14.7908 19.6517 16.8644 20.2784C17.6609 20.5191 18.4886 20.2178 18.9439 19.5215C20.1296 17.7086 19.809 17.9776 21.8002 17.1248C22.5651 16.7972 23.0055 16.0344 22.9067 15.2083C22.6497 13.0573 22.577 13.4695 23.5542 11.5363C23.9296 10.7937 23.7766 9.92631 23.17 9.35688C21.5904 7.87444 21.7997 8.23687 21.3057 6.12775C21.1159 5.31763 20.4412 4.7515 19.6104 4.70525C17.4474 4.58494 17.8407 4.72806 16.1065 3.43Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>Protocol Service</h4>
                            <p>Experience VIP treatment with our protocol services, offering personalized assistance for a seamless travel process.</p>
                        </div>
                    </li>
                    <li class="single-service">
                        <div class="icon">
                            <svg width="30" height="30" viewBox="0 0 30 30" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M20.4896 4.14404C19.8384 3.81275 19.0419 4.07233 18.7107 4.72379L8.87091 24.0772C8.53971 24.7285 8.79929 25.5249 9.45066 25.8561C9.63601 25.9507 9.84112 26 10.0492 26C10.5313 26 10.996 25.7356 11.2296 25.2763L21.0694 5.92297C21.4006 5.27161 21.141 4.47523 20.4896 4.14404ZM13.4243 10.5172C13.4243 7.75042 11.1733 5.49943 8.40653 5.49943C5.63991 5.49943 3.38892 7.75042 3.38892 10.5172C3.38892 13.2839 5.63991 15.5349 8.40662 15.5349C11.1734 15.5349 13.4243 13.2839 13.4243 10.5172ZM8.40662 12.8889C7.09894 12.8889 6.03496 11.8249 6.03496 10.5172C6.03496 9.20945 7.09894 8.14548 8.40662 8.14548C9.71438 8.14548 10.7784 9.20937 10.7784 10.5172C10.7783 11.8249 9.71438 12.8889 8.40662 12.8889ZM21.5934 14.4653C18.8266 14.4653 16.5756 16.7163 16.5756 19.483C16.5756 22.2497 18.8265 24.5007 21.5934 24.5007C24.3601 24.5007 26.6111 22.2498 26.6111 19.483C26.6111 16.7162 24.3602 14.4653 21.5934 14.4653ZM21.5934 21.8546C20.2856 21.8546 19.2216 20.7906 19.2216 19.4829C19.2216 18.1752 20.2856 17.1113 21.5934 17.1113C22.901 17.1113 23.965 18.1752 23.965 19.4829C23.965 20.7906 22.9011 21.8546 21.5934 21.8546Z"/>
                            </svg>
                        </div>
                        <div class="content">
                            <h4>Photography</h4>
                            <p>Capture your travel memories with professional photography services, preserving every moment in stunning detail.</p>
                        </div>
                    </li>
                </ul>
                
            </div>
        </div>
    </div>
    <!-- home1 service Section End-->


    <!-- home1 offer banner Section Start -->
    <div class="home1-offer-banner-section mb-100" style="background-image: linear-gradient(180deg, rgba(0, 0, 0, 0.35) 0%, rgba(0, 0, 0, 0.35) 100%), url(assets/img/home1/home1-offer-banner-bg.jpg);">
        <div class="container">
            <div class="banner-content">
                <span>Discover Africa. Live the Journey.</span>
                <h2>Your Passport to Unforgettable African Adventures Begins Here!</h2>
                <div class="author-area">
                    <h5>Nhyiraba Travel & Tour Services</h5>
                    <span>Explore • Learn • Work Abroad</span>
                </div>
                <a href="travelPackage" class="primary-btn1 two">
                    <span>
                        Start Your Journey Today
                        <svg width="10" height="10" viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.73535 1.14746C9.57033 1.97255 9.32924 3.26406 9.24902 4.66797C9.16817 6.08312 9.25559 7.5453 9.70214 8.73633C9.84754 9.12406 9.65129 9.55659 9.26367 9.70215C8.9001 9.83849 8.4969 9.67455 8.32812 9.33398L8.29785 9.26367L8.19921 8.98438C7.73487 7.5758 7.67054 5.98959 7.75097 4.58203C7.77875 4.09598 7.82525 3.62422 7.87988 3.17969L1.53027 9.53027C1.23738 9.82317 0.762615 9.82317 0.469722 9.53027C0.176829 9.23738 0.176829 8.76262 0.469722 8.46973L6.83593 2.10254C6.3319 2.16472 5.79596 2.21841 5.25 2.24902C3.8302 2.32862 2.2474 2.26906 0.958003 1.79102L0.704097 1.68945L0.635738 1.65527C0.303274 1.47099 0.157578 1.06102 0.310542 0.704102C0.463655 0.347333 0.860941 0.170391 1.22363 0.28418L1.29589 0.310547L1.48828 0.387695C2.47399 0.751207 3.79966 0.827571 5.16601 0.750977C6.60111 0.670504 7.97842 0.428235 8.86132 0.262695L9.95312 0.0585938L9.73535 1.14746Z"/>
                        </svg>
                    </span>
                    <span>
                        Start Your Journey Today
                        <svg width="10" height="10" viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.73535 1.14746C9.57033 1.97255 9.32924 3.26406 9.24902 4.66797C9.16817 6.08312 9.25559 7.5453 9.70214 8.73633C9.84754 9.12406 9.65129 9.55659 9.26367 9.70215C8.9001 9.83849 8.4969 9.67455 8.32812 9.33398L8.29785 9.26367L8.19921 8.98438C7.73487 7.5758 7.67054 5.98959 7.75097 4.58203C7.77875 4.09598 7.82525 3.62422 7.87988 3.17969L1.53027 9.53027C1.23738 9.82317 0.762615 9.82317 0.469722 9.53027C0.176829 9.23738 0.176829 8.76262 0.469722 8.46973L6.83593 2.10254C6.3319 2.16472 5.79596 2.21841 5.25 2.24902C3.8302 2.32862 2.2474 2.26906 0.958003 1.79102L0.704097 1.68945L0.635738 1.65527C0.303274 1.47099 0.157578 1.06102 0.310542 0.704102C0.463655 0.347333 0.860941 0.170391 1.22363 0.28418L1.29589 0.310547L1.48828 0.387695C2.47399 0.751207 3.79966 0.827571 5.16601 0.750977C6.60111 0.670504 7.97842 0.428235 8.86132 0.262695L9.95312 0.0585938L9.73535 1.14746Z"/>
                        </svg>
                    </span>
                </a>
            </div>
        </div>
    </div>
    <!-- home1 offer banner Section End -->


    <div class="home1-offer-package-section mb-100">
        <div class="container">
            <div class="row justify-content-center mb-50 wow animate fadeInDown" data-wow-delay="200ms" data-wow-duration="1500ms">
            <div class="col-xl-6 col-lg-8">
                <div class="section-title text-center">
                <h2>RECOMMENDED FOR YOU</h2>
                <p>A carefully selected collection of top travel packages tailored to diverse destinations.</p>
                </div>
            </div>
            </div>

            <div class="row gy-md-5 gy-4">

            <!-- ===== PACKAGE CARD 1 ===== -->
            <div class="col-lg-4 col-md-6 wow animate fadeInDown" data-wow-delay="200ms" data-wow-duration="1500ms">
                <div class="package-card">
                <div class="package-img-wrap">
                    <div class="swiper package-card-img-slider">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide">
                        <a href="travelPackageDetails" class="package-img">
                            <img src="assets/img/home1/tour-package-img7.jpg" alt="">
                        </a>
                        </div>
                        <div class="swiper-slide">
                        <a href="travelPackageDetails" class="package-img">
                            <img src="assets/img/home1/tour-package-img17.jpg" alt="">
                        </a>
                        </div>
                        <div class="swiper-slide">
                        <a href="travelPackageDetails" class="package-img">
                            <img src="assets/img/home1/tour-package-img18.jpg" alt="">
                        </a>
                        </div>
                    </div>
                    </div>
                    <div class="slider-pagi-wrap">
                    <div class="package-card-img-pagi paginations"></div>
                    </div>
                    <div class="batch">
                    <span>Hot Sale!</span>
                    </div>
                </div>
                <div class="package-content">
                    <h5><a href="travelPackageDetails">PANAFEST in Ghana</a></h5>
                    <div class="location-and-time">
                    <div class="location">
                        <svg width="14" height="14" viewBox="0 0 14 14"><path d="M6.83615 0C3.77766 0 1.28891 2.48879 1.28891 5.54892C1.28891 7.93837 4.6241 11.8351 6.05811 13.3994C6.25669 13.6175 6.54154 13.7411 6.83615 13.7411C7.13076 13.7411 7.41561 13.6175 7.6142 13.3994C9.04821 11.8351 12.3834 7.93833 12.3834 5.54892C12.3834 2.48879 9.89464 0 6.83615 0Z"/></svg>
                        <a href="travelPackage">Ghana</a>
                    </div>
                    <svg class="arrow" width="25" height="6" viewBox="0 0 25 6"><path d="M0 3L5 5.88675V0.113249L0 3ZM25 3L20 0.113249V5.88675L25 3ZM4.5 3.5H20.5V2.5H4.5V3.5Z"/></svg>
                    <span>10 Days</span>
                    </div>
                    <div class="btn-and-price-area">
                    <a href="travelPackageDetails" class="primary-btn1">
                        <span>Book Now <svg width="10" height="10" viewBox="0 0 10 10"><path d="M9.73535 1.14746C9.57033 1.97255 9.32924 3.26406 9.24902 4.66797C9.16817 6.08312 9.25559 7.5453 9.70214 8.73633C9.84754 9.12406 9.65129 9.55659 9.26367 9.70215C8.9001 9.83849 8.4969 9.67455 8.32812 9.33398L8.29785 9.26367L8.19921 8.98438C7.73487 7.5758 7.67054 5.98959 7.75097 4.58203C7.77875 4.09598 7.82525 3.62422 7.87988 3.17969L1.53027 9.53027C1.23738 9.82317 0.762615 9.82317 0.469722 9.53027C0.176829 9.23738 0.176829 8.76262 0.469722 8.46973L6.83593 2.10254C6.3319 2.16472 5.79596 2.21841 5.25 2.24902C3.8302 2.32862 2.2474 2.26906 0.958003 1.79102L0.704097 1.68945L0.635738 1.65527C0.303274 1.47099 0.157578 1.06102 0.310542 0.704102C0.463655 0.347333 0.860941 0.170391 1.22363 0.28418L1.48828 0.387695C2.47399 0.751207 3.79966 0.827571 5.16601 0.750977C6.60111 0.670504 7.97842 0.428235 8.86132 0.262695L9.95312 0.0585938L9.73535 1.14746Z"/></svg></span>
                    </a>
                    <div class="price-area">
                        <h6>Per Person</h6>
                        <span>$6,195.00</span>
                    </div>
                    </div>
                
                </div>
                </div>
            </div>

            <!-- ===== PACKAGE CARD 2 ===== -->
            <div class="col-lg-4 col-md-6 wow animate fadeInDown" data-wow-delay="300ms" data-wow-duration="1500ms">
                <div class="package-card">
                <div class="package-img-wrap">
                    <img src="assets/img/home1/tour-package-img8.jpg" alt="Safari Adventure">
                    <div class="batch"><span>Top Pick</span></div>
                </div>
                <div class="package-content">
                    <h5><a href="travelPackageDetails">Safari Adventure in Kenya</a></h5>
                    <div class="location-and-time">
                    <div class="location"><a href="#">Kenya</a></div>
                    <svg class="arrow" width="25" height="6" viewBox="0 0 25 6"><path d="M0 3L5 5.88675V0.113249L0 3ZM25 3L20 0.113249V5.88675L25 3ZM4.5 3.5H20.5V2.5H4.5V3.5Z"/></svg>
                    <span>7 Days</span>
                    </div>
                    <div class="btn-and-price-area">
                    <a href="#" class="primary-btn1">Book Now</a>
                    <div class="price-area"><h6>Per Person</h6><span>$3,850.00</span></div>
                    </div>
                </div>
                </div>
            </div>

            <!-- ===== PACKAGE CARD 3 ===== -->
            <div class="col-lg-4 col-md-6 wow animate fadeInDown" data-wow-delay="400ms" data-wow-duration="1500ms">
                <div class="package-card">
                <div class="package-img-wrap">
                    <img src="assets/img/home1/tour-package-img9.jpg" alt="Island Retreat">
                    <div class="batch"><span>Limited Offer</span></div>
                </div>
                <div class="package-content">
                    <h5><a href="travelPackageDetails">Island Retreat in Seychelles</a></h5>
                    <div class="location-and-time">
                    <div class="location"><a href="#">Seychelles</a></div>
                    <svg class="arrow" width="25" height="6" viewBox="0 0 25 6"><path d="M0 3L5 5.88675V0.113249L0 3ZM25 3L20 0.113249V5.88675L25 3ZM4.5 3.5H20.5V2.5H4.5V3.5Z"/></svg>
                    <span>5 Days</span>
                    </div>
                    <div class="btn-and-price-area">
                    <a href="#" class="primary-btn1">Book Now</a>
                    <div class="price-area"><h6>Per Person</h6><span>$2,950.00</span></div>
                    </div>
                </div>
                </div>
            </div>

            </div>
        </div>
    </div>
   
    <!-- home1 partner area Section Start-->
    <div class="partner-section mb-100">
        <div class="container">
            <div class="partner-title wow animate fadeInDown" data-wow-delay="200ms" data-wow-duration="1500ms">
                <h5>Our Parners</h5>
            </div>
            <div class="partner-wrap">
                <div class="marquee">
                    <div class="marquee__group">
                        <a href="#"><img src="assets/img/home1/partner-01.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-02.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-03.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-04.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-05.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-06.png" alt=""></a>
                    </div>
                    <div aria-hidden="true" class="marquee__group">
                        <a href="#"><img src="assets/img/home1/partner-01.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-02.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-03.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-04.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-05.png" alt=""></a>
                        <a href="#"><img src="assets/img/home1/partner-06.png" alt=""></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- home1 partner area Section End-->


    <!-- home1 testimonial Section Start-->
    <div class="home1-testimonial-section mb-100">
        <div class="container">
            <div class="row justify-content-center mb-50 wow animate fadeInDown" data-wow-delay="200ms" data-wow-duration="1500ms">
                <div class="col-xl-6 col-lg-8">
                    <div class="section-title text-center">
                        <h2>Voices from Across Africa</h2>
                        <p>From entrepreneurs to educators, innovators to community leaders—hear how we support meaningful journeys in business, education, and social impact across the continent.</p>
                    </div>
                </div>
            </div>  
            <div class="row mb-40">
                <div class="col-lg-12">
                    <div class="swiper home1-testimonial-slider">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <div class="testimonial-card">
                                    <div class="author-area">
                                        <div class="author-info">
                                            <h5>Aisha Mensah</h5>
                                            <span>Tech Entrepreneur, Accra</span>
                                        </div>
                                    </div>
                                    <h5>Seamless Business Expansion into East Africa</h5>
                                    <div class="content">
                                        <p>Relocating our startup operations to Nairobi was smooth and efficient. From legal setup to office space and local partnerships, every step was professionally managed. We're now operational in under 60 days!</p>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="testimonial-card two">
                                    <div class="author-area">
                                        <div class="author-info">
                                            <h5>Kofi Adebayo</h5>
                                            <span>Education Director, Lagos</span>
                                        </div>
                                    </div>
                                    <h5>Transformative Teacher Training Program</h5>
                                    <div class="content">
                                        <p>The cross-border teacher exchange between Nigeria and Ghana elevated our curriculum. 40 educators gained new methodologies in STEM and digital literacy. The coordination across borders was flawless.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="testimonial-card">
                                    <div class="author-area">
                                        <div class="author-info">
                                            <h5>Nia Okonkwo</h5>
                                            <span>Healthcare NGO Founder, Abidjan</span>
                                        </div>
                                    </div>
                                    <h5>Reliable Medical Supply Chain Across 5 Countries</h5>
                                    <div class="content">
                                        <p>Delivering vaccines and equipment to rural clinics in Côte d'Ivoire, Benin, and Togo required precision. Customs clearance, cold-chain logistics, and last-mile delivery were handled with zero delays.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="testimonial-card">
                                    <div class="author-area">
                                        <div class="author-info">
                                            <h5>Thabo Zulu</h5>
                                            <span>Agricultural Cooperative Leader, Johannesburg</span>
                                        </div>
                                    </div>
                                    <h5>Successful Market Access for Smallholder Farmers</h5>
                                    <div class="content">
                                        <p>Connected 200 farmers to buyers in Kenya and Zambia. The trade mission included training, certification support, and contract negotiations. First export shipment delivered on time and fully compliant.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide">
                                <div class="testimonial-card two">
                                    <div class="author-area">
                                        <div class="author-info">
                                            <h5>Fatima Diop</h5>
                                            <span>Renewable Energy Engineer, Dakar</span>
                                        </div>
                                    </div>
                                    <h5>Efficient Project Deployment in Remote Regions</h5>
                                    <div class="content">
                                        <p>Installing solar microgrids in northern Senegal required complex permitting and community engagement. The team navigated local regulations and trained technicians—now 15 villages have reliable power.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- home1 testimonial Section End-->


<?php 

include 'includes/footer.php';